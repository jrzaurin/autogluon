...
def multi_algo_state():
    def _candidate_evaluations(num):
        return [
            CandidateEvaluation(
                candidate=(i,), metrics=dictionarize_objective(float(i))
            )
            for i in range(num)
        ]
    ...
    return {
        "0": TuningJobState(
            hp_ranges=HyperparameterRanges_Impl(
                HyperparameterRangeContinuous(
                    "a1_hp_1", -5.0, 5.0, LinearScaling(), -5.0, 5.0
                )
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [CandidateEvaluation((1, "a"), {"": 9.87})],
            [(2, "b")],
            [PendingEvaluation((3, "c"))],
            {(1, "a"), (2, "b"), (3, "c")},
        ),
    ],
)
def test_compute_blacklisted_candidates(
    ...
    state = TuningJobState(
        hp_ranges, candidate_evaluations, failed_candidates, pending_evaluations
    )
...
def test_generate_unique_candidates(num_unique_candidates, num_requested_candidates):
    candidates = generate_unique_candidates(
        RepeatedCandidateGenerator(num_unique_candidates),
...
def test_generate_unique_candidates_fixed_choice(multi_algo_state):
    random_generator = RandomCandidateGenerator(multi_algo_state["0"].hp_ranges, 0)
    candidates = []
