...
def plot_performance_vs_trials(
    results, output_directory, save_file="PerformanceVsTrials.png", plot_title=""
):
    try:
    ...
    ordered_val_perfs = [
        results["trial_info"][trial_id][results["reward_attr"]]
...
def plot_summary_of_models(
    ...
    datadict["performance"] = [
        results["trial_info"][trial_id][results["reward_attr"]]
        for trial_id in datadict["trial_id"]
    ]
    datadict["hyperparameters"] = [
        _formatDict(results["trial_info"][trial_id]["config"])
        for trial_id in datadict["trial_id"]
    ]
    hidden_keys = []
    ...
    if "latency" in results["metadata"]:
        datadict["latency"] = [
            results["trial_info"][trial_id]["metadata"]["latency"]
            for trial_id in datadict["trial_id"]
        ]
        attr_x = "latency"
    else:
        ...
        datadict[attr_x] = [
            results["trial_info"][trial_id]["config"][attr_x]
            for trial_id in datadict["trial_id"]
        ]
    ...
    if "memory" in results["metadata"]:
        datadict["memory"] = [
            results["trial_info"][trial_id]["metadata"]["memory"]
            for trial_id in datadict["trial_id"]
        ]
        attr_size = "memory"
    ...
    if "training_loss" in results:
        datadict["training_loss"] = [
            results["trial_info"][trial_id]["training_loss"]
            for trial_id in datadict["trial_id"]
        ]
...
def plot_tabular_models(
    ...
    model_hyperparams = [
        _formatDict(results["model_hyperparams"][key]) for key in model_names
    ]
    datadict = {
    ...
    datadict["inference_latency"] = [
        leaderboard["pred_time_val"][leaderboard["model"] == m].values[0]
        for m in model_names
    ]
    datadict["training_time"] = [
        leaderboard["fit_time"][leaderboard["model"] == m].values[0]
        for m in model_names
    ]
    mousover_plot(
        datadict,
...
def mousover_plot(
    ...
    for key in datadict.keys():  # Check lengths are all the same
        if len(datadict[key]) != n:
            raise ValueError(
                "Key %s in datadict has different length than %s" % (key, attr_x)
            )
    ...
    if attr_color is not None:
        ...
        if attr_color_is_string:
            ...
            color_mapper = CategoricalColorMapper(
                ...
                palette=[
                    colorpalette[2 * i % len(colorpalette)]
            ...
            color_mapper = LinearColorMapper(
                palette="Magma256",
                low=min(datadict[attr_color]),
                high=max(datadict[attr_color]) * 1.25,
            )
        default_color = {"field": attr_color, "transform": color_mapper}

    if (
        attr_size is not None
    ):  # different size for each point, ensure mean-size == point_size
        attr_size2 = attr_size + "____"
        hidden_keys.append(attr_size2)
        og_sizevals = np.array(datadict[attr_size])
        sizevals = point_size + (og_sizevals - np.mean(og_sizevals)) / np.std(
            og_sizevals
        ) * (point_size / 2)
    ...
    if attr_x_is_string:  # add x-ticks:
        ...
        p.xaxis.major_label_overrides = {
            i: attr_x_levels[i] for i in range(len(attr_x_levels))
        }
    ...
    if attr_size is not None:
        p.add_layout(
            Legend(
                items=[LegendItem(label='Size of points based on "' + attr_size + '"')]
            ),
            "below",
        )
