...
for block_arg in blocks_args:
    ...
    if block_arg.num_repeat > 1:
        block_arg.update(input_filters=out_filters, stride=1, input_size=input_size)
...
class ENAS_MBNet(nn.Module):
    def __init__(
        self, features, blocks, dropout_rate=0.2, num_classes=1000, input_size=224
    ):
        super().__init__()
...
scheduler = Torch_ENAS_Scheduler(
    mbnet,
    train_set="imagenet",
    num_cpus=32,
    num_gpus=8,
