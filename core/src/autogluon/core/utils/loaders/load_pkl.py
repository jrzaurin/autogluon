...
def load(path, format=None, verbose=True, **kwargs):
    ...
    validated_path = compression_utils.get_validated_path(
        path, compression_fn=compression_fn
    )
    ...
    if compression_fn in compression_fn_map:
        with compression_fn_map[compression_fn]["open"](
            validated_path, "rb", **compression_fn_kwargs
        ) as fin:
            object = pickle.load(fin)
    else:
        raise ValueError(
            f"compression_fn={compression_fn} or compression_fn_kwargs={compression_fn_kwargs} are not valid. Valid function values: {compression_fn_map.keys()}"
        )
