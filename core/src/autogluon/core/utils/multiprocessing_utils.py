...
def dataframe_transform_parallel(df, transformer):
    cpu_count = multiprocessing.cpu_count()
    workers_count = int(round(cpu_count))
    logger.log(
        15,
        "Dataframe_transform_parallel running pool with "
        + str(workers_count)
        + " workers",
    )
    df_chunks = np.array_split(df, workers_count)
    df_list = execute_multiprocessing(
        workers_count=workers_count, transformer=transformer, chunks=df_chunks
    )
...
def execute_multiprocessing(
    workers_count, transformer, chunks, multiprocessing_method="forkserver"
):
