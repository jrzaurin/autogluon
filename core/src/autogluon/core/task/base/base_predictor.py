...
class BasePredictor(ABC):
    ...
    def __init__(self, loss_func, eval_func, model=None, results=None, **kwargs):
        ...
        self.loss_func = (
            loss_func  # Loss function (or string name) minimized during training
        )
    ...
    def _createResults(self):
        ...
        results[
            "reward_attr"
        ] = "none"  # (str), the reward attribute used to measure the performance
        results[results["reward_attr"]] = None  # performance of the best trials
        results[
            "num_trials_completed"
        ] = None  # number of trials completed during task.fit()
        results[
            "best_hyperparameters"
        ] = None  # hyperparameter values corresponding to the chosen model in self.model
        results[
            "search_space"
        ] = None  # hyperparameter search space considered in task.fit()
        results[
            "search_strategy"
        ] = None  # HPO algorithm used (ie. Hyperband, random, BayesOpt). If the HPO algorithm used kwargs, then this should be tuple (HPO_algorithm_string, HPO_kwargs)

        results[
            "metadata"
        ] = {}  # dict containing other optional metadata with keys. For example:
        ...
        results[
            ...
            {}
        )  # dict with keys = trial_IDs, values = dict of information about each individual trial (length = results['num_trials_completed'])
