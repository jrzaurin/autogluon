...
AG_TEXT_IMPORT_ERROR = (
    "autogluon.text has not been installed. "
...
class TextPredictionV1Model(AbstractModel):
    ...
    def _preprocess(self, X, fit=False, **kwargs):
        if fit:
            ...
            self._feature_generator = BulkFeatureGenerator(
                generators=[
                    [
                        CategoryFeatureGenerator(
    ...
    def _build_model(self, X, y, X_val, y_val, hyperparameters):
        try:
            from autogluon.text.text_prediction.text_prediction import (
                ag_text_prediction_params,
                merge_params,
                get_column_properties,
                infer_problem_type,
                infer_eval_stop_log_metrics,
            )
            from autogluon.text.text_prediction.models.basic_v1 import (
        ...
        problem_type, label_shape = infer_problem_type(
            column_properties=column_properties, label_col_name=self._label_column_name
        )
    ...
    def _fit(
        ...
        **kwargs,
        ...
        try:
            import mxnet as mx
            from autogluon.text.text_prediction.dataset import (
                TabularDataset,
                random_split_train_val,
            )
            from autogluon.text.text_prediction.text_prediction import (
                get_recommended_resource,
            )
        except ImportError:
            raise ImportError(AG_TEXT_IMPORT_ERROR)
        ...
        # Infer resource
        resource = get_recommended_resource(
            nthreads_per_trial=num_cpus, ngpus_per_trial=num_gpus
        )
        ...
        column_properties = self._build_model(
            X=X, y=y, X_val=X_val, y_val=y_val, hyperparameters=self.params
        )
