...
def standford_dog_resnet152_v1(
    pretrained=False,
    ...
    **kwargs
    ...
    if pretrained:
        net.load_parameters(
            get_model_file("standford_dog_resnet152_v1", root=root), ctx=ctx
        )
    return net


def standford_dog_resnext101_64x4d(
    pretrained=False,
    root=os.path.join("~", ".autogluon", "models"),
    ctx=mx.cpu(0),
    **kwargs
    ...
    if pretrained:
        net.load_parameters(
            get_model_file("standford_dog_resnext101_64x4d", root=root), ctx=ctx
        )
    return net
