...
class PipelineFeatureGenerator(BulkFeatureGenerator):
    ...
    def _fit_transform_custom(
        self, X_out: DataFrame, type_group_map_special: dict, y=None
    ) -> (DataFrame, dict):
        if len(list(X_out.columns)) == 0:
            self._is_dummy = True
            self._log(
                30,
                f"\tWARNING: No useful features were detected in the data! AutoGluon will train using 0 features, and will always predict the same value. Ensure that you are passing the correct data to AutoGluon!",
            )
            dummy_generator = DummyFeatureGenerator()
            X_out = dummy_generator.fit_transform(X=X_out)
            type_group_map_special = copy.deepcopy(
                dummy_generator.feature_metadata.type_group_map_special
            )
    ...
    def _infer_features_in_full(
        ...
        self._feature_metadata_in_real = FeatureMetadata(
            ...
            type_group_map_special=self.feature_metadata_in.get_type_group_map_raw(),
        )

    def _remove_features_in(self, features: list):
        super()._remove_features_in(features)
        if features:
            self._feature_metadata_in_real = (
                self._feature_metadata_in_real.remove_features(features=features)
            )
    ...
    def _compute_pre_memory_usage(self, X: DataFrame):
        ...
        pre_memory_usage_percent = self.pre_memory_usage / (
            available_mem + self.pre_memory_usage
        )
        self._log(
            20,
            f"\tAvailable Memory:                    {(round((self.pre_memory_usage + available_mem) / 1e6, 2))} MB",
        )
        self._log(
            20,
            f"\tTrain Data (Original)  Memory Usage: {round(self.pre_memory_usage / 1e6, 2)} MB ({round(pre_memory_usage_percent * 100, 1)}% of available memory)",
        )
        if pre_memory_usage_percent > 0.05:
            self._log(
                30,
                f"\tWarning: Data size prior to feature transformation consumes {round(pre_memory_usage_percent * 100, 1)}% of available memory. Consider increasing memory or subsampling the data to avoid instability.",
            )
    ...
    def print_feature_metadata_info(self, log_level=20):
        if self._useless_features_in:
            ...
            self._log(
                log_level,
                f"\t\tThese features carry no predictive signal and should be manually investigated.",
            )  # TODO: What about features with 1 unique value but also np.nan?
            self._log(
                log_level,
                f"\t\tThis is typically a feature which has the same value for all rows.",
            )
            self._log(
                log_level,
                f"\t\tThese features do not need to be present at inference time.",
            )
        if self._feature_metadata_in_unused.get_features():
            ...
            self._log(
                log_level,
                f"\tUnused Original Features (Count: {len(self._feature_metadata_in_unused.get_features())}): {self._feature_metadata_in_unused.get_features()}",
            )
            self._log(
                log_level,
                f"\t\tThese features were not used to generate any of the output features. Add a feature generator compatible with these features to utilize them.",
            )
            self._log(
                log_level,
                f"\t\tFeatures can also be unused if they carry very little information, such as being categorical but having almost entirely unique values or being duplicates of other features.",
            )
            self._log(
                log_level,
                f"\t\tThese features do not need to be present at inference time.",
