...
class CategoricalOrdinalEnc(EncBase):
    ...
    def fit(self, data: pd.Series):
        ...
        if (
            len(sorted_vals) >= 0.5 * len(data) or max(Counter(data).values()) < 10
...
class ScalarQuantileOrdinalEnc(EncBase):
    def __init__(self, n_bins_=None, bin_edges_=None):
        if n_bins_ is not None and bin_edges_ is not None:
            ...
            self.disc.bin_edges_ = np.array(
                [np.array(bin_edges_), np.array(bin_edges_[:-1])]
            )[
                :1
...
class ScalarRescaleEnc(EncBase):
    ...
    def enc_cont(self, scalars):
        ...
        vals = (
            self.scaler.transform(vals) + 1e-7
        )  # Extra 1e-7 to help with correctness testing
        vals[null_idxs] = 0
...
class ScalarPowerTransformerEnc(ScalarRescaleEnc):
    def __init__(
        self, lambdas_=None, scale_=None, mean_=None, var_=None, n_samples_seen_=None
    ):
...
class EmbeddingInitializer(nn.Module):
    def __init__(
        ...
        if out_dim is not None:
            ...
            ), "Makes no sense: just set max_emb_dim = out_dim and out_dim = None"
        ...
        self.embed = nn.Embedding(
            num_embeddings=num_embeddings + 1, embedding_dim=self.emb_dim, padding_idx=0
        )
        ...
        if shared_embedding:
            ...
            ce_dim = (
                self.emb_dim
                if shared_embedding_added
                else (out_dim if out_dim else self.emb_dim) // n_shared_embs
            )  # used to be //8
