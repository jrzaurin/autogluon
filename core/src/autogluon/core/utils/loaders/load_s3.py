...
def list_bucket_prefix_suffix_contains_s3(
    bucket, prefix, suffix=None, banned_suffixes=None, contains=None
):
    if banned_suffixes is None:
        banned_suffixes = []
    s3 = boto3.resource("s3")
    ...
    for object_summary in my_bucket.objects.filter(Prefix=prefix):
        ...
        if (
            (not is_banned)
            and ((suffix is None) or (suffix in suffix_full))
            and (contains is None or contains in suffix_full)
        ):
            files.append(object_summary.key)
...
def load_multipart_s3(
    bucket, prefix, columns_to_keep=None, dtype=None, sample_count=None
):
