...
from ..vectorizers import (
    get_ngram_freq,
    downscale_vectorizer,
    vectorizer_auto_ml_default,
)
...
class TextNgramFeatureGenerator(AbstractFeatureGenerator):
    ...
    **kwargs :
    ...
    def _fit_transform(
        self, X: DataFrame, y: Series = None, problem_type: str = None, **kwargs
    ) -> (DataFrame, dict):
        ...
        if self.prefilter_tokens and problem_type not in ["binary", "regression"]:
            logger.warning(
                "`prefilter_tokens` was enabled but invalid `problem_type`. Disabling `prefilter_tokens`."
            )
            self.prefilter_tokens = False
        ...
        if self.prefilter_tokens:
            ...
            X_out = X_out[
                X_out.columns[self.token_mask]
            ]  # select the columns that are most correlated with y
    ...
    def _transform(self, X: DataFrame) -> DataFrame:
        ...
        try:
            ...
            if self.prefilter_tokens:
                X_out = X_out[
                    X_out.columns[self.token_mask]
                ]  # select the columns identified during training
        except Exception:
            self._log(
                40,
    ...
    def _fit_transform_ngrams(self, X):
        ...
        self._log(
            20,
            f"Fitting {self.vectorizer_default_raw.__class__.__name__} for text features: "
            + str(self.features_in),
            self.log_prefix + "\t",
        )
        ...
        for nlp_feature in self.vectorizer_features:
            ...
            if nlp_feature == "__nlp__":  # Combine Text Fields
                text_list = list(
                    set([". ".join(row) for row in X[self.features_in].values])
                )
            else:
                text_list = list(X[nlp_feature].drop_duplicates().values)
            vectorizer_raw = copy.deepcopy(self.vectorizer_default_raw)
            try:
                vectorizer_fit, _ = self._train_vectorizer(
                    text_list, vectorizer_raw
                ...
                self._log(
                    30,
                    f"Removing text_ngram feature due to error: '{nlp_feature}'",
                    self.log_prefix + "\t",
                )
        ...
        while keep_trying_nlp:
            try:
                X_text_ngram = self._generate_ngrams(
                    X=X, downsample_ratio=downsample_ratio
                )
    ...
    def _generate_ngrams(self, X, downsample_ratio: int = None):
        ...
        for nlp_feature, vectorizer_fit in zip(
            self.vectorizer_features, self.vectorizers
        ):
            if nlp_feature == "__nlp__":
            ...
            X_nlp_features = pd.DataFrame(
                transform_matrix.toarray()
            )  # TODO: Consider keeping sparse
            X_nlp_features.columns = [f"{nlp_feature}.{x}" for x in nlp_features_names]
            X_nlp_features[nlp_feature + "._total_"] = (
                X_nlp_features.gt(0).sum(axis=1).astype(np.int32)
            )
    ...
    def _adjust_vectorizer_memory_usage(
        ...
        predicted_ngrams_memory_usage_bytes = (
            len(text_data) * 4 * (transform_matrix.shape[1] + 1) + 80
        )
        ...
        if downsample_ratio is None:
            if (
                self.max_memory_ratio is not None
        ...
        if downsample_ratio is not None:
            if (downsample_ratio >= 1) or (downsample_ratio <= 0):
                raise ValueError(
                    f"downsample_ratio must be >0 and <1, but downsample_ratio is {downsample_ratio}"
                )
            vocab_size = len(vectorizer_fit.vocabulary_)
            downsampled_vocab_size = int(np.floor(vocab_size * downsample_ratio))
            self._log(
                20,
                f"Reducing Vectorizer vocab size from {vocab_size} to {downsampled_vocab_size} to avoid OOM error",
            )
            ngram_freq = get_ngram_freq(
                vectorizer=vectorizer_fit, transform_matrix=transform_matrix
            )
            downscale_vectorizer(
                vectorizer=vectorizer_fit,
                ngram_freq=ngram_freq,
                vocab_size=downsampled_vocab_size,
            )
    ...
    def _train_vectorizer(text_data: list, vectorizer):
        transform_matrix = vectorizer.fit_transform(
            text_data
        )  # TODO: Consider upgrading to pandas 0.25.0 to benefit from sparse attribute improvements / bug fixes! https://pandas.pydata.org/pandas-docs/stable/whatsnew/v0.25.0.html
    ...
    def _remove_features_in(self, features):
        ...
        if features:
            self.vectorizer_features = [
                feature
                for feature in self.vectorizer_features
