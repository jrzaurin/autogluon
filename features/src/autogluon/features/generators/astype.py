    ...
    def _transform(self, X: DataFrame) -> DataFrame:
        ...
        if int_features:
            ...
            if len(with_null) > 0:
                ...
                logger.warning(
                    f"WARNING: Int features without null values at train time contain null values at inference time! Imputing nulls to 0. To avoid this, pass the features as floats during fit!"
                )
                logger.warning(
                    f"WARNING: Int features with nulls: {with_null_features}"
                )
