...
class TabTransformer(TabModelBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(self.cat_feat_origin_cards, list):
            self.n_embeddings = len(self.cat_feat_origin_cards) + (
                n_cont_embeddings if self.n_cont_features else 0
            )
        else:
        ...
        if self.n_cont_features:
            ...
            self.cont_initializer = nn.Linear(
                self.n_cont_features, hidden_dim * n_cont_embeddings
            )
    ...
    def init_input(self, input):
        feats = [
            init(input[:, i]) for i, init in enumerate(self.cat_initializers.values())
        ]

        if self.readout == "readout_emb":
            readout_emb = self.readout_emb.expand_as(feats[0])
            feat_embs = torch.stack(
                [readout_emb] + feats, dim=0
            )  # (n_feat_embeddings + 1) x batch x hidden_dim
        else:
            feat_embs = torch.stack(
                feats, dim=0
            )  # n_feat_embeddings x batch x hidden_dim
        return feat_embs
    ...
    def run_tfmr(self, feat_embs):
        ...
        if self.readout == "readout_emb":
            ...
            for each_feat_embs in [all_feat_embs[0], all_feat_embs[-1]]:
                feat_embs_all_layers.append(
                    each_feat_embs.transpose(0, 1).reshape(each_feat_embs.shape[1], -1)
                )
            ...
            last_layer = feat_embs.transpose(0, 1).reshape(feat_embs.shape[1], -1)
            last_layer += orig_feat_embs_cp.transpose(0, 1).reshape(
                orig_feat_embs_cp.shape[1], -1
            )
