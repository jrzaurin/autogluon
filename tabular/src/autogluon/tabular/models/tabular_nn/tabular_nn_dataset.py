...
from autogluon.core.utils.multiprocessing_utils import (
    is_fork_enabled,
    is_forkserver_enabled,
)
...
class TabularNNDataset:
    ...
    DATAVALUES_SUFFIX = (
        "_tabNNdata.npz"  # This file contains raw data values as data_list of NDArrays
    )
    ...
    def __init__(
        ...
        self.num_features = len(
            feature_arraycol_map
        )  # number of features (!=dim(processed_array) because some features may be vector-valued, eg one-hot)
        ...
        if feature_arraycol_map.keys() != feature_type_map.keys():
            raise ValueError(
                "feature_arraycol_map and feature_type_map must share same keys"
            )
        self.feature_groups = {
            "vector": [],
            "embed": [],
            "language": [],
        }  # maps feature_type -> list of feature_names (order is preserved in list)
        ...
        data_list = (
            []
        )  # stores all data of each feature-type in list used to construct MXNet dataset. Each index of list = 2D NDArray.
        self.label_index = (
            None  # int describing which element of the dataset._data list holds labels
        )
        ...
        self.vecfeature_col_map = (
            {}
        )  # maps vector_feature_name ->  columns of dataset._data[vector] array that contain data for this feature
        self.feature_dataindex_map = (
            {}
        )  # maps feature_name -> i such that dataset._data[i] = data array for this feature. Cannot be used for vector-valued features, instead use: self.vecfeature_col_map

        if len(self.feature_groups["vector"]) > 0:
            vector_inds = []  # columns of processed_array corresponding to vector data
            for feature in feature_type_map:
                if feature_type_map[feature] == "vector":
                    current_last_ind = len(
                        vector_inds
                    )  # current last index of the vector datamatrix
                    vector_inds += feature_arraycol_map[feature]
                    new_last_ind = len(
                        vector_inds
                    )  # new last index of the vector datamatrix
                    self.vecfeature_col_map[feature] = list(
                        range(current_last_ind, new_last_ind)
                    )
            data_list.append(
                mx.nd.array(processed_array[:, vector_inds], dtype="float32")
            )  # Matrix of data from all vector features
        ...
        if len(self.feature_groups["embed"]) > 0:
            for feature in feature_type_map:
                if feature_type_map[feature] == "embed":
                    ...
                    data_list.append(
                        mx.nd.array(processed_array[:, feature_colind], dtype="int32")
                    )  # array of ints with data for this embedding feature
        ...
        if len(self.feature_groups["language"]) > 0:
            for feature in feature_type_map:
                if feature_type_map[feature] == "language":
                    ...
                    data_list.append(
                        mx.nd.array(processed_array[:, feature_colinds], dtype="int32")
                    )  # array of ints with data for this language feature
        ...
        if labels is not None:
            ...
            self.label_index = len(
                data_list
            )  # To access data labels, use: self.dataset._data[self.label_index]
            self.num_classes = None
            if self.problem_type == SOFTCLASS:
                ...
                if self.problem_type == REGRESSION and labels.dtype != np.float32:
                    labels = labels.astype(
                        "float32"
                    )  # Convert to proper float-type if not already
                elif self.problem_type in [BINARY, MULTICLASS]:
                    self.num_classes = len(set(labels))
                data_list.append(mx.nd.array(labels.reshape(len(labels), 1)))

        self.embed_indices = [
            i for i in range(len(self.data_desc)) if "embed" in self.data_desc[i]
        ]  # list of indices of embedding features in self.dataset, order matters!
        self.language_indices = [
            i for i in range(len(self.data_desc)) if "language" in self.data_desc[i]
        ]  # list of indices of language features in self.dataset, order matters!
    ...
    def generate_dataset_and_dataloader(self, data_list):
        self.dataset = mx.gluon.data.dataset.ArrayDataset(
            *data_list
        )  # Access ith embedding-feature via: self.dataset._data[self.data_desc.index('embed_'+str(i))].asnumpy()
    ...
    def getNumCategoriesEmbeddings(self):
        ...
        if self.num_categories_per_embed_feature is not None:
            ...
            for i in range(num_embed_feats):
                ...
                num_categories_i = len(
                    set(feat_i_data)
                )  # number of categories for ith feature
    ...
    def get_feature_data(self, feature, asnumpy=True):
        ...
        if self.feature_type_map[feature] == "vector":
            vector_datamatrix = self.dataset._data[
                self.vectordata_index
            ]  # does not work for one-hot...
    ...
    def format_batch_data(self, data_batch, ctx):
        ...
        if not isinstance(data_batch, list):
            data_batch = [
                data_batch
            ]  # Need to convert to list if dimension was dropped during batching
        ...
        if self.has_vector_features():  # None if there is no vector data
            formatted_batch["vector"] = data_batch[self.vectordata_index].as_in_context(
                ctx
            )
    ...
    def save(self, file_prefix=""):
        ...
        logger.debug(
            "TabularNN Dataset saved to files: \n %s \n %s"
    ...
    def load(cls, file_prefix=""):
        ...
        logger.debug(
            "TabularNN Dataset loaded from files: \n %s \n %s"
            % (dataobj_file, datalist_file)
        )
        return dataset
