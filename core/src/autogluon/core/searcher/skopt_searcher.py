...
class SKoptSearcher(BaseSearcher):
    ...
    def __init__(self, configspace, **kwargs):
        ...
        self.hp_ordering = (
            configspace.get_hyperparameter_names()
        ...
        for hp in self.hp_ordering:
            ...
            if "integer" in hp_type:
                hp_dimension = Integer(
                    low=int(hp_obj.lower), high=int(hp_obj.upper), name=hp
                )
            elif "float" in hp_type:
                if hp_obj.log:  # log10-scale hyperparmeter
                    hp_dimension = Real(
                        low=float(hp_obj.lower),
                    ...
                    hp_dimension = Real(
                        low=float(hp_obj.lower), high=float(hp_obj.upper), name=hp
                    )
    ...
    def get_config(self, **kwargs):
        ...
        if (
            len(self._results) == 0
        ):  # no hyperparams have been tried yet, first try default config
            return self.default_config()
        try:
            new_points = self.bayes_optimizer.ask(
                n_points=1
            )  # initially ask for one new config
            new_config_cs = self.skopt2config(
                new_points[0]
            )  # hyperparameter-config to evaluate
            try:
                new_config_cs.is_valid_configuration()
                new_config = new_config_cs.get_dictionary()
                if (
                    pickle.dumps(new_config) not in self._results.keys()
                ):  # have not encountered this config
                    self._results[
                        pickle.dumps(new_config)
                    ] = self._reward_while_pending()
                    return new_config
