...
class SliceSampler(object):
    def __init__(
        self, log_density: Callable[[np.ndarray], float], scale: float, random_seed: int
    ):
    ...
    def _gen_next_sample(self, x0: np.ndarray) -> np.ndarray:
        ...
        lower_bound, upper_bound = slice_sampler_step_out(
            log_pivot, self.scale, sliced_log_density, self.random_state
        )
        movement = slice_sampler_step_in(
            lower_bound, upper_bound, log_pivot, sliced_log_density, self.random_state
        )
...
def gen_random_direction(
    dimension: int, random_state: np.random.RandomState
) -> np.ndarray:
