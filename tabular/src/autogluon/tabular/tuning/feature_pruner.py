...
class FeaturePruner:
    ...
    def tune(self, X, y, X_val, y_val, X_holdout, y_holdout, total_runs=999):
        ...
        for iteration in range(self.cur_iteration, total_runs):
            ...
            if feature_importance is not None:
                ...
                logger.log(
                    15,
                    "Unused features after pruning: "
                    + str(list(unused_features.index)),
                )
                features_to_use = [
                    feature
                    for feature in valid_features
                    if feature not in unused_features.index
                ]
            ...
            logger.log(
                15, "Iter " + str(iteration) + "  Score Val: " + str(cur_score_val)
            )
            ...
            gain_df = model_iter.compute_feature_importance(
                X=X_val_subset, y=y_val, features=features_to_use
            )
            ...
            valid_features = [
                feature for feature in valid_features if feature not in banned_features
            ]
    ...
    def adjust_threshold(gain_df, threshold):
        ...
        if len(banned_features) == 0:
            ...
            return FeaturePruner.adjust_threshold(
                gain_df=gain_df, threshold=threshold_new
            )
        else:
            return threshold
