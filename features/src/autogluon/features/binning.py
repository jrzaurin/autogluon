...
def generate_bins(X_features: DataFrame, features_to_bin: list, ideal_bins: int = 10):
    ...
    starting_cats = 1000
    bin_index_starting = [
        np.floor(X_len * (num + 1) / starting_cats) for num in range(starting_cats - 1)
    ]
    ...
    for column in features_to_bin:
        ...
        bins_value_counts = X_features[column].value_counts(
        ...
        if max_bins <= ideal_bins:
            ...
            interval_index = get_bins(
                bins=bins, bin_index=bin_index, bin_epsilon=bin_epsilon
            )
        else:
            cur_len = X_len
            bins = X_features[column].sort_values(ascending=True)
            interval_index = get_bins(
                bins=bins, bin_index=bin_index_starting, bin_epsilon=bin_epsilon
            )
        ...
        is_satisfied = (len(interval_index) >= min_desired_bins) and (
            len(interval_index) <= max_desired_bins
        )
        ...
        while not is_satisfied:
            ...
            bin_index = [
                np.floor(cur_len * (num + 1) / num_cats_current)
                for num in range(num_cats_current - 1)
            ]
            interval_index = get_bins(
                bins=bins, bin_index=bin_index, bin_epsilon=bin_epsilon
            )

            if (len(interval_index) >= min_desired_bins) and (
                len(interval_index) <= max_desired_bins
            ):
                is_satisfied = True
