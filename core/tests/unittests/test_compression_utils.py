...
def test_get_validated_path_no_compression_fn():
    ...
    assert (
        compression_utils.get_validated_path(no_extension_filepath)
        == no_extension_filepath
    )

    single_extension_filepath = "dummy_file.pkl"
    assert (
        compression_utils.get_validated_path(single_extension_filepath)
        == single_extension_filepath
    )

    multiple_extension_filepath = "dummy_file.fake-foo.zip.pkl"
    assert (
        compression_utils.get_validated_path(multiple_extension_filepath)
        == multiple_extension_filepath
    )
...
def test_get_validated_path_with_compression_fn():
    ...
    assert (
        compression_utils.get_validated_path(no_extension_filepath, compression_fns[2])
    ...
    assert (
        compression_utils.get_validated_path(
            single_extension_filepath, compression_fns[0]
        )
        == expected_pkl_gzip_filepath
    )
    assert (
        compression_utils.get_validated_path(
            single_extension_filepath, compression_fns[1]
        )
        == expected_pkl_lzma_filepath
    )
    assert (
        compression_utils.get_validated_path(
            single_extension_filepath, compression_fns[2]
        )
    ...
    assert (
        compression_utils.get_validated_path(
            multiple_extension_filepath, compression_fns[0]
        )
        == expected_multi_gzip_filepath
    )
    assert (
        compression_utils.get_validated_path(
            multiple_extension_filepath, compression_fns[1]
        )
        == expected_multi_lzma_filepath
    )
    assert (
        compression_utils.get_validated_path(
            multiple_extension_filepath, compression_fns[2]
        )
