...
def record_evaluation_custom(
    path, eval_result, interval, offset=0, early_stopping_rounds=None
):
    ...
    def _callback(env):
        ...
        if (
            (interval > 0)
            and ((env.iteration - offset) % interval == 0)
            and (env.iteration != 0)
        ):
...
def early_stopping_custom(
    ...
    def _init(env):
        ...
        if verbose:
            ...
            if manual_stop_file:
                logger.debug(
                    "Manually stop training by creating file at location: ",
                    manual_stop_file,
                )
    ...
    def _callback(env):
        ...
        if train_loss_name is not None:
            train_loss_evals = [
                eval
                for eval in env.evaluation_result_list
        ...
        for i in indices_to_check:
            ...
            ):  # Report current best scores for iteration, used in HPO
                if (
        ...
        if env.iteration % 10 == 0:
            ...
            if verbose or (model_size_memory_ratio > 0.25):
                ...
                logging.debug(
                    "Estimated Model Size: " + str(estimated_model_size_mb) + " MB"
                )
            ...
            if model_size_memory_ratio > 1.0:
                ...
                logger.warning(
                    "Estimated GBM model size: " + str(estimated_model_size_mb) + " MB"
                )
                early_stop = True
            ...
            if early_stop:
                ...
                logger.log(
                    ...
                    % (
                        best_iter[0] + 1,
                        "\t".join([_format_eval_result(x) for x in best_score_list[0]]),
                    ),
                )
