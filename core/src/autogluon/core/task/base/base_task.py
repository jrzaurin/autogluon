...
class BaseTask(object):
    ...
    @classmethod
    def run_fit(cls, train_fn, search_strategy, scheduler_options, plot_results=False):
        ...
        if plot_results or in_ipynb():
            plot_training_curves = scheduler_options["checkpoint"].replace(
                "exp1.ag", "plot_training_curves.png"
            )
            scheduler.get_training_curves(
                filename=plot_training_curves, plot=True, use_legend=False
            )
        record_args = copy.deepcopy(args)
        if results is None:
            logger.warning(
                "No valid results obtained with best config, the result may not be useful..."
            )
            results = {}
        results.update(
...
def compile_scheduler_options_v2(
    ...
    (
        scheduler_params["resource"]["num_cpus"],
        scheduler_params["resource"]["num_gpus"],
    ) = setup_compute(
    ...
    if missing_options:
        raise AssertionError(
            f"Missing required keys in scheduler_options: {missing_options}"
        )
