...
def try_import_catboostdev():  # TODO: remove once Catboost 0.24 is released.
    try:
        ...
        raise ImportError(
            "`import catboost_dev` failed (needed for distillation with CatBoost models). "
...
def try_import_fastai_v1():
    try:
        ...
        assert (
            parse_version("1.0.61") <= fastai_version < parse_version("2.0.0")
        ), "Currently, we only support 1.0.61<=fastai<2.0.0"
    except ModuleNotFoundError as e:
        raise ImportError(
            "Import fastai failed. A quick tip is to install via `pip install fastai==1.*`. "
...
def try_import_gluonnlp():
    try:
        ...
        assert gluonnlp_version >= parse_version(
            "0.8.1"
        ) and gluonnlp_version <= parse_version(
            "0.8.3"
...
def try_import_skopt():
    try:
        ...
        raise ImportError(
            "`import skopt` failed. skopt is an optional dependency and may not be installed.\n"
