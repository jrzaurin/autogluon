...
def get_dataset(args):
    ...
    if "voc" in args.dataset_name:
        logging.info(
            "Please follow this instruction to download dataset: \
    ...
    test_dataset = ObjectDetector.Dataset.from_voc(
        data_root, splits=index_file_name_test
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-format", type=str, default="voc", help="dataset format"
    )
    parser.add_argument(
        "--index-file-name-trainval",
        type=str,
        default="",
        help="name of txt file which contains images for training and validation ",
    )
    parser.add_argument(
        "--index-file-name-test",
        type=str,
        default="",
    ...
    parser.add_argument(
        "--num-trials", type=int, default=3, help="the HPO trials to perform"
    )
    parser.add_argument(
        "--meta-arch",
        type=str,
    ...
    if args.meta_arch == "yolo3":
        transfer = (
            None
            if ("voc" in args.dataset_name) or ("coco" in args.dataset_name)
            else ag.Categorical("yolo3_darknet53_coco", "yolo3_mobilenet1.0_coco")
        )
        hyperparameters = {
        ...
        transfer = (
            ...
            if ("voc" in args.dataset_name) or ("coco" in args.dataset_name)
                ...
                "faster_rcnn_fpn_resnet101_v1d_coco", "faster_rcnn_resnet50_v1b_coco"
            )
        )
        hyperparameters = {
            "estimator": args.meta_arch,
            ...
            "data_shape": (640, 800),
            "lr_decay_epoch": ag.Categorical(
                [24, 28], [35], [50, 55], [40], [45], [55], [30, 35], [20]
            ),
