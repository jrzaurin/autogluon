...
class StackerEnsembleModel(BaggedEnsembleModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_models_dict: Dict[
            str, AbstractModel
        ] = base_models_dict  # String name -> Model objects
        ...
        if (base_model_performances_dict is not None) and (
            base_model_types_inner_dict is not None
        ):
    ...
    @staticmethod
    def limit_models_per_type(
        models, model_types, model_scores, max_base_models_per_type
    ):
        ...
        for key in model_type_groups:
            models_remain += sorted(
                model_type_groups[key], key=lambda x: x[1], reverse=True
    ...
    def limit_models(self, models, model_scores, max_base_models):
        ...
        return self.limit_models_per_type(
            ...
            max_base_models_per_type=max_base_models,
        )
        ...
        if self.stack_column_prefix_lst:
            ...
            if compute_base_preds:
                ...
                for stack_column_prefix in self.stack_column_prefix_lst:
                    base_model_name = self.stack_column_prefix_to_model_map[
                        stack_column_prefix
                    ]
                    if fit:
                        ...
                        and base_model_name in model_pred_proba_dict
                    ):
    ...
    def pred_probas_to_df(self, pred_proba: list, index=None) -> pd.DataFrame:
        if self.problem_type in [MULTICLASS, SOFTCLASS]:
            ...
            pred_proba = pd.DataFrame(
                data=np.asarray(pred_proba).T, columns=self.stack_columns
            )
    ...
    def _hyperparameter_tune(
        self, X, y, k_fold, scheduler_options, compute_base_preds=True, **kwargs
    ):
    ...
    def _add_stack_to_feature_metadata(self):
        if len(self.models) == 0:
            ...
            if (
                ...
                existing_stack_features = self.feature_metadata.get_features(
                    required_special_types=[S_STACK]
                )
                if set(stacker_feature_metadata.get_features()) != set(
                    existing_stack_features
                ):
