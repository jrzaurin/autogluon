...
class NumericBlock(gluon.HybridBlock):
    ...
    def __init__(self, params, **kwargs):
        ...
        with self.name_scope():
            self.body = gluon.nn.Dense(
                params["numeric_embed_dim"], activation=params["activation"]
            )
...
class EmbedBlock(gluon.HybridBlock):
    ...
    def __init__(self, embed_dim, num_categories, **kwargs):
        ...
        with self.name_scope():
            self.body = gluon.nn.Embedding(
                input_dim=num_categories,
                output_dim=embed_dim,
                weight_initializer=mx.init.Orthogonal(scale=0.1, rand_type="uniform"),
            )  # for Xavier-style: scale = np.sqrt(3/float(embed_dim))
    ...
    def __init__(self, params, num_net_outputs, **kwargs):
        ...
        with self.name_scope():
            ...
            for i in range(len(layers)):
                ...
                self.body.add(
                    gluon.nn.Dense(layer_width, activation=params["activation"])
                )
                if params["use_batchnorm"]:
                    self.body.add(gluon.nn.BatchNorm())
                if params["dropout_prob"] > 0:
...
class EmbedNet(gluon.Block):  # TODO: hybridize?
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (architecture_desc is None) and (
            train_dataset is None or params is None or num_net_outputs is None
        ):
            raise ValueError(
                "train_dataset, params, num_net_outputs cannot = None if architecture_desc=None"
            )
        super(EmbedNet, self).__init__(**kwargs)
        if (
            architecture_desc is None
            ...
            if self.has_embed_features:
                ...
                embed_dims = getEmbedSizes(
                    train_dataset, params, num_categs_per_feature
                )
        ...
        if self.has_embed_features:
            ...
            for i in range(len(num_categs_per_feature)):
                self.embed_blocks.add(
                    EmbedBlock(embed_dims[i], num_categs_per_feature[i])
                )
        ...
        if params["network_type"] == "feedforward":
            ...
            raise ValueError(
                "unknown network_type specified: %s" % params["network_type"]
            )

        y_range = params[
            "y_range"
        ]  # Used specifically for regression. = None for classification.
        ...
        if y_range is not None:
            ...
            self.y_lower = nd.array(params["y_range"][0]).reshape(
                1,
            )
            self.y_upper = nd.array(params["y_range"][1]).reshape(
                1,
            )
            if ctx is not None:
        ...
        if architecture_desc is None:  # Save Architecture description
            ...
            if self.has_language_features:
                self.architecture_desc[
                    "text_TODO"
                ] = None  # TODO: store text architecture
    ...
    def forward(self, data_batch):
        ...
        if self.has_embed_features:
            ...
            for i in range(1, len(self.embed_blocks)):
                embed_activations = nd.concat(
                    embed_activations, self.embed_blocks[i](embed_data[i]), dim=2
                )
        ...
        if self.has_language_features:
            ...
            language_activations = self.text_block(
                language_data
            )  # TODO: create block to embed text fields
    ...
    embed_dims = [
        int(
            ...
            * max(
                2,
                min(
                    max_embedding_dim, 1.6 * num_categs_per_feature[i] ** embed_exponent
                ),
            )
        )
        for i in range(len(num_categs_per_feature))
