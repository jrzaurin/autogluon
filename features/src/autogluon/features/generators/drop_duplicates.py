...
class DropDuplicatesFeatureGenerator(AbstractFeatureGenerator):
    ...
    **kwargs :
    ...
    def _fit_transform(self, X: DataFrame, **kwargs) -> (DataFrame, dict):
        if self.sample_size_init is not None and len(X) > self.sample_size_init:
            features_to_check = self._drop_duplicate_features(
                ...
                keep=False,
                sample_size=self.sample_size_init,
            )
        ...
        if features_to_drop:
            self._log(
                15,
                f"\t{len(features_to_drop)} duplicate columns removed: {features_to_drop}",
            )
    ...
    def _drop_duplicate_features(
        ...
        features_to_check_numeric = feature_metadata_in.get_features(
            valid_raw_types=[R_INT, R_FLOAT]
        )
        features_to_check_numeric = [
            feature for feature in features_to_check_numeric if feature in X_columns
        ]
        if features_to_check_numeric:
            features_to_remove += cls._drop_duplicate_features_numeric(
                X=X[features_to_check_numeric], keep=keep
            )
            X = X.drop(columns=features_to_check_numeric)

        X_columns = set(X.columns)
        features_to_check_categorical = feature_metadata_in.get_features(
            valid_raw_types=[R_CATEGORY, R_BOOL]
        )
        features_to_check_categorical = [
            feature for feature in features_to_check_categorical if feature in X_columns
        ]
    ...
    @classmethod
    def _drop_duplicate_features_generic(
        cls, X: DataFrame, keep: Union[str, bool] = "first"
    ):
        ...
        features_to_remove = [
            column for column in X_columns if column not in features_to_keep
        ]
        return features_to_remove
    ...
    def _drop_duplicate_features_numeric(
        ...
        for key in feature_sum_map:
            ...
            features_to_keep = set(
                X[feature_sum_map[key]].T.drop_duplicates(keep=keep).T.columns
            )
            features_to_remove += [
                feature
                for feature in feature_sum_map[key]
    ...
    @classmethod
    def _drop_duplicate_features_categorical(
        cls, X: DataFrame, keep: Union[str, bool] = "first"
    ):
        """
        ...
        for feature in X_columns:
            ...
            mapping_features_val_dict[feature] = dict(
                zip(feature_unique_vals, range(len(feature_unique_vals)))
            )
        ...
        for feature_unique_count in features_unique_count_dict:
            ...
            mapping_features_val_dict_cur = {
                feature: mapping_features_val_dict[feature]
