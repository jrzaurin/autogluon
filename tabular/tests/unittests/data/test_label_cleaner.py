...
def test_label_cleaner_binary():
    ...
    label_cleaner = LabelCleaner.construct(
        problem_type=problem_type, y=input_labels
    )  # positive_class='l2'
    label_cleaner_pos_class_l1 = LabelCleaner.construct(
        problem_type=problem_type, y=input_labels, positive_class="l1"
    )

    # Raise exception
    with pytest.raises(ValueError):
        LabelCleaner.construct(
            problem_type=problem_type, y=input_labels, positive_class="unknown_class"
        )
    ...
    output_labels_new_pos_class_l1 = label_cleaner_pos_class_l1.transform(
        input_labels_new
    )

    output_labels_inverse = label_cleaner.inverse_transform(output_labels)
    output_labels_inverse_pos_class_l1 = label_cleaner_pos_class_l1.inverse_transform(
        output_labels_pos_class_l1
    )
    ...
    output_labels_new_inverse_pos_class_l1 = (
        label_cleaner_pos_class_l1.inverse_transform(output_labels_new_pos_class_l1)
    )
    ...
    assert expected_output_labels_new_pos_class_l1.equals(
    ...
    assert expected_output_labels_new_inverse.equals(
        output_labels_new_inverse_pos_class_l1
    )
...
def test_label_cleaner_multiclass():
    ...
    # When
    label_cleaner = LabelCleaner.construct(
        problem_type=problem_type, y=input_labels, y_uncleaned=input_labels
    )
...
def test_label_cleaner_regression():
    ...
    # When
    label_cleaner = LabelCleaner.construct(
        problem_type=problem_type, y=input_labels, y_uncleaned=None
    )
