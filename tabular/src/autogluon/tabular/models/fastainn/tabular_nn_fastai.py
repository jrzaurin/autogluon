...
from autogluon.core.features.types import (
    R_OBJECT,
    R_INT,
    R_FLOAT,
    R_DATETIME,
...
class NNFastAiTabularModel(AbstractModel):
    ...
    def _preprocess_train(self, X, y, X_val, y_val, num_workers):
        ...
        if self.problem_type == REGRESSION and self.y_scaler is not None:
            y_norm = pd.Series(
                self.y_scaler.fit_transform(y.values.reshape(-1, 1)).reshape(-1)
            )
            y_val_norm = (
                pd.Series(
                    self.y_scaler.transform(y_val.values.reshape(-1, 1)).reshape(-1)
                )
    ...
    def _preprocess(self, X: pd.DataFrame, fit=False, **kwargs):
        ...
        if fit:
            self.cat_columns = self.feature_metadata.get_features(
                valid_raw_types=[R_OBJECT, R_CATEGORY, R_BOOL]
            )
            self.cont_columns = self.feature_metadata.get_features(
            ...
            try:
                ...
                cat_cols_to_drop = X_stats[
                    (
                        X_stats["unique"]
                        > self.params.get("max_unique_categorical_values", 10000)
                    )
                    | (X_stats["unique"].isna())
                ]["index"].values
            except:
                cat_cols_to_drop = []
            cat_cols_to_keep = [
                col for col in X.columns.values if (col not in cat_cols_to_drop)
            ]
            cat_cols_to_use = [
            ...
            self.cat_columns = [
                feature for feature in self.cat_columns if feature in list(X.columns)
            ]
            self.cont_columns = [
                feature for feature in self.cont_columns if feature in list(X.columns)
            ]
    ...
    def _fit(
        ...
        **kwargs,
        ...
        best_epoch_stop = params.get(
            "best_epoch", None
        )  # Use best epoch for refit_full.
        ...
        with make_temp_directory() as temp_dir:
            ...
            with progress_disabled_ctx(self.model) as model:
                ...
                model.fit_one_cycle(
                    params["epochs"], params["lr"], callbacks=save_callback
                )
    ...
    def __get_objective_func_name(self):
        ...
        if objective_func_name not in metrics_map.keys():
            ...
            logger.warning(
                f"Metric {self.stopping_metric.name} is not supported by this model - using {objective_func_name} instead"
            )
    ...
    def save(self, path: str = None, verbose=True) -> str:
        ...
        save_pkl.save_with_fn(
            ...
            self.model,
            lambda m, buffer: m.export(buffer),
            verbose=verbose,
        )
        return path_final
