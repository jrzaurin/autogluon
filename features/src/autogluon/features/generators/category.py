    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cat_order not in ["original", "alphanumeric", "count"]:
            raise ValueError(
                f"cat_order must be one of {['original', 'alphanumeric', 'count']}, but was: {cat_order}"
            )
        ...
        if fillna is not None:
            if fillna not in ["mode"]:
                raise ValueError(
                    f"fillna={fillna} is not a valid value. Valid values: {[None, 'mode']}"
                )
        self._fillna = fillna
        ...
        if minimize_memory:
            self._post_generators = [
                CategoryMemoryMinimizeFeatureGenerator(inplace=True)
            ] + self._post_generators

    def _fit_transform(self, X: DataFrame, **kwargs) -> (DataFrame, dict):
        if self._stateful_categories:
            X_out, self.category_map, self._fillna_map = self._generate_category_map(
                X=X
            )
        ...
        if S_TEXT in feature_metadata_out_type_group_map_special:
            ...
            feature_metadata_out_type_group_map_special[S_TEXT_AS_CATEGORY] += [
                ...
                if feature
                not in feature_metadata_out_type_group_map_special[S_TEXT_AS_CATEGORY]
            ]
    ...
    def _generate_features_category(self, X: DataFrame) -> DataFrame:
        if self.features_in:
            ...
            if self.category_map is not None:
                X_category = copy.deepcopy(
                    X_category
                )  # TODO: Add inplace version / parameter
                for column in self.category_map:
                    X_category[column].cat.set_categories(
                        self.category_map[column], inplace=True
                    )
                if self._fillna_map is not None:
                    for column in self._fillna_map:
                        X_category[column].fillna(
                            self._fillna_map[column], inplace=True
                        )
        else:
                ...
                if (
                    ...
                    if len(category_list) > 1:
                        if self.cat_order == "original":
                            ...
                            category_list = [
                                cat
                                for cat in original_cat_order
                                if cat in set_category_list
                            ]
                    ...
                    X_category[column] = X_category[column].cat.reorder_categories(
                        category_list
                    )
                elif self.cat_order == "alphanumeric":
