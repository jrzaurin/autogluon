...
class LocalReporter:
    ...
    def __init__(
        self, trial, searcher_config, training_history: dict, config_history: dict
    ):
        self.trial = trial
...
class LocalSequentialScheduler(object):
    ...
    def init_limits_(self, kwargs):
        if kwargs.get("num_trials", None) is None:
            assert (
                kwargs.get("time_out", None) is not None
    ...
    def get_searcher_(self, searcher, train_fn, **kwargs) -> BaseSearcher:
        ...
        if isinstance(searcher, str):
            ...
            searcher = searcher_factory(
                searcher, **{**scheduler_opts, **_search_options}
            )
        else:
    ...
    def run(self, **kwargs):
        ...
        for i in tqdm(r) if self.num_trials < 1000 else r:
            ...
            trial_run_times.append(
                np.NaN if is_failed else (trial_end_time - trial_start_time)
            )
            ...
            if self.time_out is not None:
                ...
                avg_trial_run_time = (
                    0 if np.isnan(avg_trial_run_time) else avg_trial_run_time
                )
    ...
    def has_enough_time_for_trial_(
        ...
        if avg_trial_run_time:
            is_enough_time_for_another_trial = (
                time_left > avg_trial_run_time * fill_factor
            )
    ...
    def run_trial(self, task_id=0) -> Tuple[bool, float, float]:
        ...
        reporter = LocalReporter(
            task_id, searcher_config, self.training_history, self.config_history
        )
