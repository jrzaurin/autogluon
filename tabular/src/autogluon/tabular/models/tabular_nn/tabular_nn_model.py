...
warnings.filterwarnings(
    "ignore", module="sklearn.preprocessing"
...
class TabularNeuralNetModel(AbstractNeuralNetworkModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.features_to_drop = (
            []
        )  # may change between different bagging folds. TODO: consider just removing these from self.features if it works with bagging
    ...
    def set_net_defaults(self, train_dataset, params):
        ...
        if (self.problem_type == MULTICLASS) or (self.problem_type == SOFTCLASS):
            ...
            if params["y_range"] is None:  # Infer default y-range
                y_vals = train_dataset.dataset._data[
                    train_dataset.label_index
                ].asnumpy()
        ...
        if params["layers"] is None:  # Use default choices for MLP architecture
            if self.problem_type == REGRESSION:
                ...
                base_size = max(
                    1, min(self.num_net_outputs, 100) / 50
                )  # TODO: Updated because it improved model quality and made training far faster
                default_layer_sizes = [
                    defaultsize * base_size for defaultsize in default_sizes
                ]
            ...
            params["layers"] = [
                int(min(max_layer_width, layer_expansion_factor * defaultsize))
                for defaultsize in default_layer_sizes
            ]

        if (
            train_dataset.has_vector_features() and params["numeric_embed_dim"] is None
        ):  # Use default choices for numeric embedding size
            vector_dim = train_dataset.dataset._data[
                train_dataset.vectordata_index
            ].shape[
                1
            ...
            params["numeric_embed_dim"] = int(
                min(
                    max_numeric_embed_dim,
                    max(
                        min_numeric_embed_dim,
                        params["layers"][0]
                        * prop_vector_features
                        * np.log10(vector_dim + 10),
                    ),
                )
            )
        return
    ...
    def _fit(
        ...
        **kwargs,
        ...
        if sample_weight is not None:  # TODO: support
            logger.log(
                15,
                "sample_weight not yet supported for TabularNeuralNetModel, this model will ignore them in training.",
            )
        ...
        if time_limit is not None:
            ...
            if (
                time_limit <= time_limit_orig * 0.4
    ...
    def train_net(
        ...
        logger.log(
            15, "Training neural network for up to %s epochs..." % params["num_epochs"]
        )
        ...
        if setup_trainer:
            ...
            if visualizer == "tensorboard" or visualizer == "mxboard":
                ...
                self.summary_writer = SummaryWriter(
                    logdir=self.path, flush_secs=5, verbose=False
                )
            self.optimizer = self.setup_trainer(
                params=params, train_dataset=train_dataset
            )
        best_val_metric = -np.inf  # higher = better
        val_metric = None
        best_val_epoch = 0
        val_improve_epoch = (
            0  # most recent epoch where validation-score strictly improved
        )
        ...
        if params["loss_function"] is None:
            if self.problem_type == REGRESSION:
                ...
                params["loss_function"] = mx.gluon.loss.SoftmaxCrossEntropyLoss(
                    sparse_label=False, from_logits=self.model.from_logits
                )
            else:
                params["loss_function"] = mx.gluon.loss.SoftmaxCrossEntropyLoss(
                    from_logits=self.model.from_logits
                )

        loss_func = params["loss_function"]
        epochs_wo_improve = params["epochs_wo_improve"]
        loss_scaling_factor = (
            1.0  # we divide loss by this quantity to stabilize gradients
        )

        rescale_losses = {
            mx.gluon.loss.L1Loss: "std",
            mx.gluon.loss.HuberLoss: "std",
            mx.gluon.loss.L2Loss: "var",
        }  # dict of loss names where we should rescale loss, value indicates how to rescale.
        ...
        if loss_torescale:
            ...
            if rescale_losses[loss_torescale] == "std":
                loss_scaling_factor = (
                    np.std(train_dataset.get_labels()) / 5.0 + EPS
                )  # std-dev of labels
            elif rescale_losses[loss_torescale] == "var":
                loss_scaling_factor = (
                    np.var(train_dataset.get_labels()) / 5.0 + EPS
                )  # variance of labels
            else:
                raise ValueError(
                    "Unknown loss-rescaling type %s specified for loss_func==%s"
                    % (rescale_losses[loss_torescale], loss_func)
                )
        ...
        if (
            num_epochs == 0
        ):  # use dummy training loop that stops immediately (useful for using NN just for data preprocessing / debugging)
            logger.log(
                20,
                "Not training Neural Net since num_epochs == 0.  Neural network architecture is:",
            )
        ...
        for e in range(num_epochs):
            ...
            train_loss = cumulative_loss / float(
                train_dataset.num_examples
            )  # training loss this epoch
            if val_dataset is not None:
                val_metric = self.score(
                    X=val_dataset, y=y_val, metric=self.stopping_metric
                )
                if np.isnan(val_metric):
                    if e == 0:
                        raise RuntimeError(
                            "NaNs encountered in TabularNeuralNetModel training. Features/labels may be improperly formatted or NN weights may have diverged."
                        )
                    else:
                        logger.warning(
                            "Warning: NaNs encountered in TabularNeuralNetModel training. Reverting model to last checkpoint without NaNs."
                        )
                        break
            ...
            if val_dataset is not None:
                ...
                if self.summary_writer is not None:
                    self.summary_writer.add_scalar(
                        tag="val_" + self.stopping_metric.name,
                        value=val_metric,
                        global_step=e,
                    )
            else:
                if verbose_eval > 0 and e % verbose_eval == 0:
                    logger.log(
                        15, "Epoch %s.  Train loss: %s" % (e, train_loss.asscalar())
                    )
            if self.summary_writer is not None:
                self.summary_writer.add_scalar(
                    tag="train_loss", value=train_loss.asscalar(), global_step=e
                )  # TODO: do we want to keep mxboard support?
            if reporter is not None:
                ...
                ):  # TODO: This might work without the if statement
            ...
            if time_limit is not None:
                ...
                if time_left < time_epoch_average:
                    logger.log(
                        20,
                        f"\tRan out of time, stopping training early. (Stopping on epoch {e})",
                    )
                    break
        ...
        if val_dataset is None:
            ...
            logger.log(
                15,
                "Best model found in epoch %d. Val %s: %s"
    ...
    def _predict_proba(self, X, **kwargs):
        ...
        if isinstance(X, TabularNNDataset):
            return self._predict_tabular_data(
                new_data=X, process=False, predict_proba=True
            )
        elif isinstance(X, pd.DataFrame):
            X = self.preprocess(X, **kwargs)
            return self._predict_tabular_data(
                new_data=X, process=True, predict_proba=True
            )
        else:
            raise ValueError(
                "X must be of type pd.DataFrame or TabularNNDataset, not type: %s"
                % type(X)
            )

    def _predict_tabular_data(
        self, new_data, process=True, predict_proba=True
    ):  # TODO ensure API lines up with tabular.Model class.
        ...
        if process:
            new_data = self.process_test_data(
                new_data,
                batch_size=self.batch_size,
                num_dataloading_workers=self.num_dataloading_workers_inference,
                labels=None,
            )
        if not isinstance(new_data, TabularNNDataset):
            raise ValueError(
                "new_data must of of type TabularNNDataset if process=False"
            )
    ...
    def generate_datasets(self, X, y, params, X_val=None, y_val=None):
        ...
        if isinstance(X, TabularNNDataset):
            ...
            train_dataset = self.process_train_data(
                ...
                labels=y,
                batch_size=self.batch_size,
                num_dataloading_workers=self.num_dataloading_workers,
        ...
        if X_val is not None:
            if isinstance(X_val, TabularNNDataset):
                ...
                val_dataset = self.process_test_data(
                    df=X_val,
                    labels=y_val,
                    batch_size=self.batch_size,
                    num_dataloading_workers=self.num_dataloading_workers_inference,
    ...
    def process_test_data(self, df, batch_size, num_dataloading_workers, labels=None):
        ...
        if labels is not None and len(labels) != len(df):
            raise ValueError(
                "Number of examples in Dataframe does not match number of labels"
            )
        if (
        ...
        df = self.processor.transform(
            df
        )  # 2D numpy array. self.feature_arraycol_map, self.feature_type_map have been previously set while processing training data.
        return TabularNNDataset(
            df,
            self.feature_arraycol_map,
            self.feature_type_map,
            batch_size=batch_size,
            num_dataloading_workers=num_dataloading_workers,
    ...
    def process_train_data(
        ...
        if set(df.columns) != set(self.features):
            raise ValueError(
                "Column names in provided Dataframe do not match self.features"
            )
        if labels is None:
        ...
        logger.log(
            15, "AutoGluon Neural Network infers features are of the following types:"
        )
        logger.log(15, json.dumps(self._types_of_features, indent=4))
        logger.log(15, "\n")
        self.processor = self._create_preprocessor(
            impute_strategy=impute_strategy, max_category_levels=max_category_levels
        )
        df = self.processor.fit_transform(df)  # 2D numpy array
        self.feature_arraycol_map = self._get_feature_arraycol_map(
            max_category_levels=max_category_levels
    ...
    def setup_trainer(self, params, train_dataset=None):
        ...
        optimizer_opts = {
            ...
            "clip_gradient": params["clip_gradient"],
        }
        if "lr_scheduler" in params and params["lr_scheduler"] is not None:
            if train_dataset is None:
                raise ValueError(
                    "train_dataset cannot be None when lr_scheduler is specified."
                )
            ...
            lr_decay_epoch = [
                max(warmup_epochs, int(params["num_epochs"] / 3)),
                max(warmup_epochs + 1, int(params["num_epochs"] / 2)),
        ...
        if params["optimizer"] == "sgd":
            ...
            optimizer = mx.gluon.Trainer(
                self.model.collect_params(), "sgd", optimizer_opts
            )
        elif params["optimizer"] == "adam":  # TODO: Can we try AdamW?
            optimizer = mx.gluon.Trainer(
                self.model.collect_params(), "adam", optimizer_opts
            )
        else:
    ...
    def _get_feature_arraycol_map(self, max_category_levels):
        ...
        feature_preserving_transforms = set(
            ["continuous", "skewed", "ordinal", "language"]
            ...
            if transformer_name in feature_preserving_transforms:
                for feature in transformed_features:
                    if feature in feature_arraycol_map:
                        raise ValueError(
                            "same feature is processed by two different column transformers: %s"
                            % feature
                        )
                ...
                oh_encoder = [
                    step for (name, step) in transformer[1].steps if name == "onehot"
                ][0]
                for i in range(len(transformed_features)):
                    ...
                    oh_dimensionality = min(
                        len(oh_encoder.categories_[i]), max_category_levels + 1
                    )
                    feature_arraycol_map[feature] = list(
                        range(current_colindex, current_colindex + oh_dimensionality)
                    )
                    current_colindex += oh_dimensionality
            else:
                raise ValueError(
                    "unknown transformer encountered: %s" % transformer_name
                )
        return OrderedDict(
            [(key, feature_arraycol_map[key]) for key in feature_arraycol_map]
        )
    ...
    def _get_feature_type_map(self):
        ...
        if self.feature_arraycol_map is None:
            raise ValueError(
                "must first call _get_feature_arraycol_map() before _get_feature_type_map()"
            )
        vector_features = (
            self._types_of_features["continuous"]
            + self._types_of_features["skewed"]
            + self._types_of_features["onehot"]
        )
    ...
    def _create_preprocessor(self, impute_strategy, max_category_levels):
        ...
        if self.processor is not None:
            Warning(
                "Attempting to process training data for TabularNeuralNetModel, but previously already did this."
            )
        ...
        transformers = (
            []
        )  # order of various column transformers in this list is important!
            ...
            onehot_transformer = Pipeline(
                steps=[
                    ...
                    (
                        "imputer",
                        SimpleImputer(
                            strategy="constant", fill_value=self.unique_category_str
                        ),
                    ),
                    (
                        "onehot",
                        OneHotMergeRaresHandleUnknownEncoder(
                            max_levels=max_category_levels, sparse=False
                        ),
                    ),
                ]
        ...
        if (
            embed_features
        ):  # Ordinal transformer applied to convert to-be-embedded categorical features to integer levels
    ...
    def load(cls, path: str, reset_paths=True, verbose=True):
        model: TabularNeuralNetModel = super().load(
            path=path, reset_paths=reset_paths, verbose=verbose
        )
        if model._architecture_desc is not None:
            from .embednet import EmbedNet

            model.model = EmbedNet(
                architecture_desc=model._architecture_desc, ctx=model.ctx
            ...
            model.model.load_parameters(
                model.path + model.params_file_name, ctx=model.ctx
            )
    ...
    def _hyperparameter_tune(self, X, y, X_val, y_val, scheduler_options, **kwargs):
        ...
        train_dataset, val_dataset = self.generate_datasets(
            X=X, y=y, params=params_copy, X_val=X_val, y_val=y_val
        )
        ...
        if not np.any(
            ...
            for hyperparam in params_copy:
                if isinstance(params_copy[hyperparam], Space):
                    logger.log(
                        15, str(hyperparam) + ":   " + str(params_copy[hyperparam])
                    )
        ...
        if ("dist_ip_addrs" in scheduler_params) and (
            len(scheduler_params["dist_ip_addrs"]) > 0
        ):
    ...
    def reduce_memory_size(self, remove_fit=True, requires_save=True, **kwargs):
        super().reduce_memory_size(
            remove_fit=remove_fit, requires_save=requires_save, **kwargs
        )
