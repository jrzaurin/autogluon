...
class ENAS_Scheduler(object):
    ...
    def __init__(
        ...
        update_arch_frequency=20,
        checkname="./enas/checkpoint.ag",
        plot_frequency=0,
        **kwargs
    ):
        ...
        self.initialize_miscs(
            train_set, val_set, batch_size, num_cpus, num_gpus, train_args, val_args
        )
        ...
        self.searcher = RLSearcher(
            kwspaces, controller_type=controller_type, prefetch=4, num_workers=4
        )
        # controller setup
    ...
    def initialize_miscs(
        self, train_set, val_set, batch_size, num_cpus, num_gpus, train_args, val_args
    ):
        ...
        if isinstance(train_set, str):
            train_set = get_built_in_dataset(
                dataset_name,
                train=True,
        ...
        if isinstance(train_set, gluon.data.Dataset):
            ...
            self.val_data = DataLoader(
                ...
                num_workers=num_cpus,
                prefetch=0,
                sample_times=self.controller_batch_size,
            )
        else:
        ...
        iters_per_epoch = (
            ...
            else IMAGENET_TRAINING_SAMPLES // batch_size
        )
        self.train_args = (
            init_default_train_args(
                batch_size, self.supernet, self.epochs, iters_per_epoch
            )
            if len(train_args) == 0
            else train_args
        )
        ...
        self.val_args["batch_fn"] = (
            imagenet_batch_fn if dataset_name == "imagenet" else default_batch_fn
        )
        self.train_args["ctx"] = ctx
        self.train_args["batch_fn"] = (
            imagenet_batch_fn if dataset_name == "imagenet" else default_batch_fn
        )
        self.ctx = ctx
    ...
    def run(self):
        ...
        for epoch in tq:
            ...
            for batch in tbar:
                ...
                self.train_fn(
                    epoch, self.epochs, self.supernet, batch, **self.train_args
                )
                mx.nd.waitall()
                if (
                    epoch >= self.warmup_epochs
                    and (idx % self.update_arch_frequency) == 0
                ):
                    self.train_controller()
                if (
                    self.plot_frequency > 0
                    and idx % self.plot_frequency == 0
                    and in_ipynb()
                ):
    ...
    def _async_sample(self):
        with mx.autograd.record():
            ...
            configs, log_probs, entropies = self.controller.sample(
                batch_size=self.controller_batch_size, with_details=True
            )
    ...
    def train_controller(self):
        ...
        with mx.autograd.record():
            ...
            for i, batch in enumerate(self.val_data):
                if i >= self.controller_batch_size:
                    break
                ...
                # substract baseline
                avg_rewards = mx.nd.array(
                    [reward - self.baseline], ctx=self.controller.context
                )
                # EMA baseline
