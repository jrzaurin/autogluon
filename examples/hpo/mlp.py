...
def parse_args():
    ...
    parser.add_argument("--debug", action="store_true", help="debug if needed")
    ...
    parser.add_argument("--random_seed", type=int, default=31415927, help="random seed")
    ...
    parser.add_argument("--num_trials", type=int, help="number of trial tasks")
    parser.add_argument(
        "--scheduler_timeout",
        type=float,
        default=120,
    ...
    parser.add_argument("--brackets", type=int, help="number of brackets")
    parser.add_argument(
        "--reduction_factor", type=int, help="Reduction factor for successive halving"
    )
    parser.add_argument(
        "--grace_period",
        type=int,
        help="minimum number of epochs to run with" "hyperband_* scheduler",
    )
    parser.add_argument(
        "--use_single_rung_system",
        action="store_true",
...
def from_argparse(args) -> (dict, dict):
    ...
    if args.scheduler != "fifo":
        ...
        _enter_not_none(scheduler_options, "reduction_factor", args.reduction_factor)
...
RESOURCE_ATTR_NAME = (
    "epoch"  # how do we measure resources   (will become clearer further)
)
REWARD_ATTR_NAME = (
    "objective"  # how do we measure performance (will become clearer further)
)


def create_train_fn(X_train, X_valid, y_train, y_valid, n_classes, epochs):
    @ag.args(
        n_units_1=ag.space.Int(lower=16, upper=128),
        n_units_2=ag.space.Int(lower=16, upper=128),
        dropout_1=ag.space.Real(lower=0, upper=0.75),
        dropout_2=ag.space.Real(lower=0, upper=0.75),
    ...
    def run_mlp_openml(args, reporter):
        ...
        trainer = gluon.Trainer(
            net.collect_params(), "adam", {"learning_rate": learning_rate}
        )
        ...
        for epoch in range(args.epochs):
            ...
            print(
                "Epoch %d ; Time: %f ; Training: %s=%f ; Validation: %s=%f"
