    ...
    **kwargs,
    ...
    if problem_type == BINARY:  # convert to regression in distillation
        ...
        if isinstance(rf_hyperparameters, list):
            ...
            rf_hyperparameters = [
                j
                for n, j in enumerate(rf_hyperparameters)
                if j not in rf_hyperparameters[(n + 1) :]
    ...
    logger.log(
        20,
        f"Distilling with each of these student models: {[model.name for model in models]}",
    )
