...
from .hyperparameters.parameters import (
    get_param_baseline,
    get_model_params,
    get_default_params,
    INCLUDE,
...
from .lr_preprocessing_utils import (
    NlpDataPreprocessor,
    OheFeaturesGenerator,
    NumericDataPreprocessor,
)
...
class LinearModel(AbstractModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.model_class, self.penalty, self.handle_text = get_model_params(
            self.problem_type, self.params
        )
        self.types_of_features = None
        self.pipeline = None

        self.model_params, default_params = get_default_params(
            self.problem_type, self.penalty
        )
    ...
    def _get_types_of_features(self, df):
        ...
        language_featnames = (
            []
        )  # TODO: Disabled currently, have to pass raw text data features here to function properly
        valid_features = (
            categorical_featnames + continuous_featnames + language_featnames
        )
        if (
            len(categorical_featnames)
            + len(continuous_featnames)
        ...
        if len(feature_types["language"]) > 0:
            pipeline = Pipeline(
                steps=[
                    (
                        "preparator",
                        NlpDataPreprocessor(nlp_cols=feature_types["language"]),
                    ),
                    (
                        "vectorizer",
                        TfidfVectorizer(
        ...
        if len(feature_types["onehot"]) > 0:
            pipeline = Pipeline(
                steps=[
                    (
                        "generator",
                        OheFeaturesGenerator(cats_cols=feature_types["onehot"]),
                    ),
                ]
            )
        ...
        if len(feature_types["continuous"]) > 0:
            pipeline = Pipeline(
                steps=[
                    (
                        "generator",
                        NumericDataPreprocessor(cont_cols=feature_types["continuous"]),
                    ),
                    (
                        "imputer",
                        SimpleImputer(strategy=self.params["proc.impute_strategy"]),
                    ),
        ...
        if len(feature_types["skewed"]) > 0:
            pipeline = Pipeline(
                steps=[
                    (
                        "generator",
                        NumericDataPreprocessor(cont_cols=feature_types["skewed"]),
                    ),
                    (
                        "imputer",
                        SimpleImputer(strategy=self.params["proc.impute_strategy"]),
    ...
    def _fit(
        ...
        **kwargs,
        ...
        X = self.preprocess(
            X, is_train=True, vect_max_features=hyperparams["vectorizer_dict_size"]
        )
    ...
    def _select_features_handle_text_include(
        ...
        for feature in self.features:
            ...
            elif (feature in categorical_featnames) and (
                num_unique_vals <= one_hot_threshold
            ):
