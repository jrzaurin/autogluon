...
def load(
    ...
    if format == "pointer":
        ...
        paths = [
            ...
            if (isfile(join(path, f))) & (f.startswith("part-"))
        ]
        return load_multipart(
            paths=paths,
            delimiter=delimiter,
        ...
        try:
            df = pd.read_parquet(
                path, columns=columns_to_keep, engine="fastparquet"
            )  # TODO: Deal with extremely strange issue resulting from torch being present in package, will cause read_parquet to either freeze or Segmentation Fault when performing multiprocessing
        except:
...
def load_multipart(
    ...
    df_list = multiprocessing_utils.execute_multiprocessing(
        ...
        transformer=load_multipart_child,
        chunks=full_chunks,
        multiprocessing_method=multiprocessing_method,
    )
    ...
    logger.log(
        20,
        "Loaded data from multipart file | Columns = "
        + str(column_count)
        + " | Rows = "
        + str(row_count),
    )
...
def load_multipart_s3(
    ...
    paths_full = [
        s3_utils.s3_bucket_prefix_to_path(bucket=bucket, prefix=file, version="s3")
