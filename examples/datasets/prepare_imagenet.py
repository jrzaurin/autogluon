...
def parse_args():
    ...
    parser.add_argument(
        "--checksum", action="store_true", help="If check integrity before extracting."
    )
    parser.add_argument(
        "--with-rec", action="store_true", help="If build image record files."
    )
    parser.add_argument(
        "--num-thread",
        type=int,
...
def main():
    ...
    extract_train(
        train_tar_fname, os.path.join(target_dir, "train"), build_rec, args.num_thread
    )
    extract_val(
        val_tar_fname, os.path.join(target_dir, "val"), build_rec, args.num_thread
    )
