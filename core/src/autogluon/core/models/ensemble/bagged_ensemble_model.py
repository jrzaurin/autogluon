...
class BaggedEnsembleModel(AbstractModel):
    ...
    def oof_pred_proba(self):
        ...
        return self._oof_pred_proba_func(
            self._oof_pred_proba, self._oof_pred_model_repeats
        )

    @staticmethod
    def _oof_pred_proba_func(oof_pred_proba, oof_pred_model_repeats):
        oof_pred_model_repeats_without_0 = np.where(
            oof_pred_model_repeats == 0, 1, oof_pred_model_repeats
        )
    ...
    def _fit(
        ...
        **kwargs,
        ...
        if k_fold == 1:
            ...
            model_base.set_contexts(
                path_context=self.path + model_base.name + os.path.sep
            )
        ...
        for j in range(n_repeat_start, n_repeats):  # For each n_repeat
            ...
            for i in range(fold_start_n_repeat, fold_end_n_repeat):  # For each fold
                fold_num_in_repeat = i - (
                    j * k_fold
                )  # The fold in the current repeat set (first fold in set = 0)
    ...
    def predict_proba(self, X, normalize=None, **kwargs):
        ...
        pred_proba = model.predict_proba(
            X=X, preprocess_nonadaptive=False, normalize=normalize
        )
    ...
    def score_with_oof(self, y, sample_weight=None):
        ...
        return self.score_with_y_pred_proba(
            y=y, y_pred_proba=y_pred_proba, sample_weight=sample_weight
        )
    ...
    def compute_feature_importance(
        self, X, y, features=None, is_oof=True, time_limit=None, silent=False, **kwargs
    ) -> pd.DataFrame:
        if features is None:
        ...
        for val in fi_fold_list:
            val = val[
                "importance"
            ].to_dict()  # TODO: Don't throw away stddev information of children
            for key in val:
    ...
    def convert_to_refit_full_template(self):
        ...
        init_args["hyperparameters"][
            "save_bag_folds"
        ] = True  # refit full models must save folds
    ...
    def convert_to_refitfull_template_child(self):
        ...
        child_compressed.feature_metadata = (
            self.feature_metadata
        )  # TODO: Don't pass this here
    ...
    def _get_compressed_params(self, model_params_list=None):
        if model_params_list is None:
            model_params_list = [
                self.load_child(child).get_trained_params() for child in self.models
            ]

        model_params_compressed = dict()
        for param in model_params_list[0].keys():
            model_param_vals = [
                model_params[param] for model_params in model_params_list
            ]
    ...
    def _get_compressed_params_trained(self):
        model_params_list = [
            self.load_child(child).params_trained for child in self.models
        ]
    ...
    @classmethod
    def load(
        cls, path: str, reset_paths=True, low_memory=True, load_oof=False, verbose=True
    ):
    ...
    def load_oof(cls, path, verbose=True):
        try:
            oof = load_pkl.load(
                path=path + "utils" + os.path.sep + cls._oof_filename, verbose=verbose
            )
        ...
        return cls._oof_pred_proba_func(
    ...
    def _load_oof(self):
        if self._oof_pred_proba is not None:
            ...
            oof = load_pkl.load(
                path=self.path + "utils" + os.path.sep + self._oof_filename
            )
    ...
    def persist_child_models(self, reset_paths=True):
        for i, model_name in enumerate(self.models):
            if isinstance(model_name, str):
                ...
                child_model = self._child_type.load(
                    path=child_path, reset_paths=reset_paths, verbose=True
                )
                self.models[i] = child_model

    def load_model_base(self):
        return load_pkl.load(
            path=self.path + "utils" + os.path.sep + "model_template.pkl"
        )
    ...
    def save(self, path=None, verbose=True, save_oof=True, save_children=False) -> str:
        ...
        if save_oof and self._oof_pred_proba is not None:
            save_pkl.save(
                path=path + "utils" + os.path.sep + self._oof_filename,
                object={
    ...
    def get_info(self):
        ...
        if self.low_memory:
            ...
            min_memory_size = (
                info["memory_size"] - sum_memory_size_child + max_memory_size_child
            )
        ...
        child_features_full = list(
            set().union(*[child["features"] for child in children_info.values()])
        )
    ...
    def _hyperparameter_tune(
        self, X, y, k_fold, scheduler_options, preprocess_kwargs=None, **kwargs
    ):
