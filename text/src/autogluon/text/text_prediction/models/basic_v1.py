...
from autogluon_contrib_nlp.utils.misc import (
    logging_config,
    grouper,
    count_parameters,
    repeat,
    get_mxnet_available_ctx,
)
...
def get_optimizer(cfg, updates_per_epoch):
    ...
    if cfg.lr_scheduler == "triangular":
        ...
        warmup_steps = int(
            updates_per_epoch * cfg.num_train_epochs * cfg.warmup_portion
        )
        lr_scheduler = InverseSquareRootScheduler(
            warmup_steps=warmup_steps, base_lr=cfg.lr, warmup_init_lr=cfg.begin_lr
        )
        ...
        raise ValueError('Unsupported lr_scheduler="{}"'.format(cfg.lr_scheduler))
    optimizer_params = {
...
@use_np
def _classification_regression_predict(
    net, dataloader, problem_type, has_label=True, extract_embedding=False
):
    """
...
def train_function(
    ...
    gt_dev_labels = np.array(
        tuning_data[label].apply(column_properties[label].transform)
    )
    ...
    logger.info(
        "#Total Params/Fixed Params={}/{}".format(
            num_total_params, num_total_fixed_params
        )
    )
    # Initialize the optimizer
    updates_per_epoch = int(len(train_dataloader) / (num_accumulated * len(ctx_l)))
    optimizer, optimizer_params, max_update = get_optimizer(
        cfg.optimization, updates_per_epoch=updates_per_epoch
    )
    ...
    trainer = mx.gluon.Trainer(
        net.collect_params(), optimizer, optimizer_params, update_on_kvstore=False
    )
    ...
    for update_idx in tqdm.tqdm(range(max_update), disable=None):
        ...
        total_norm, ratio, is_finite = clip_grad_global_norm(
            params, cfg.optimization.max_grad_norm * num_accumulated
        )
        ...
        if (update_idx + 1) % valid_interval == 0 or (update_idx + 1) == max_update:
            ...
            dev_predictions = _classification_regression_predict(
                net,
            ...
            log_scores = [
                calculate_metric(
                    scorer, gt_dev_labels, dev_predictions, problem_types[0]
                )
                for scorer in log_metric_scorers
            ]
            dev_score = calculate_metric(
                stopping_metric_scorer, gt_dev_labels, dev_predictions, problem_types[0]
            )
            valid_time_spent = time.time() - valid_start_tick

            if (
                best_performance_score is None
                or (greater_is_better and dev_score >= best_performance_score)
                or (not greater_is_better and dev_score <= best_performance_score)
            ):
                find_better = True
                no_better_rounds = 0
...
class BertForTextPredictionBasic:
    ...
    def train(
        ...
        tuning_df_path = os.path.join(
            self._output_directory, "cache_tuning_dataframe.pq"
        )
        ...
        best_model_saved_dir_path = os.path.join(
            self._output_directory, "task{}".format(best_task_id)
        )
        ...
        if plot_results:
            plot_training_curves = os.path.join(
                self._output_directory, "plot_training_curves.png"
            )
            scheduler.get_training_curves(
                filename=plot_training_curves, plot=plot_results, use_legend=True
            )
        ...
        ctx_l = get_mxnet_available_ctx()
        net.load_parameters(
            os.path.join(best_model_saved_dir_path, "best_model.params"), ctx=ctx_l
        )
        self._net = net
        mx.npx.waitall()
    ...
    def evaluate(self, valid_data, metrics):
        ...
        ground_truth = np.array(
            valid_data.table[self._label_columns[0]].apply(
        ...
        metric_scores = {
            metric: calculate_metric(
                get_metric(metric), ground_truth, predictions, self.problem_types[0]
            )
    ...
    def _internal_predict(
        self, test_data, get_original_labels=True, get_probabilities=False
    ):
        assert self.net is not None
        ...
        inference_batch_size = (
            self.config.optimization.per_device_batch_size
    ...
    def predict_proba(self, test_data):
        ...
        return self._internal_predict(
            test_data, get_original_labels=False, get_probabilities=True
        )
    ...
    def predict(self, test_data, get_original_labels=True):
        ...
        """
        return self._internal_predict(
            test_data, get_original_labels=get_original_labels, get_probabilities=False
        )
    ...
    def save(self, dir_path):
        ...
        with open(os.path.join(dir_path, "column_metadata.json"), "w") as of:
            json.dump(
                get_column_property_metadata(self._column_properties),
                of,
                ensure_ascii=True,
            )
    ...
    def load(cls, dir_path: str):
        ...
        """
        loaded_config = cls.default_config().clone_merge(
            os.path.join(dir_path, "cfg.yml")
        )
    ...
    def extract_embedding(self, data):
        ...
        embeddings = _classification_regression_predict(
            self._embed_net,
            dataloader=dataloader,
            problem_type=self._problem_types[0],
            has_label=False,
