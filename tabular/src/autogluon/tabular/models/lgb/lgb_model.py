...
class LGBModel(AbstractModel):
    ...
    def _set_default_params(self):
        default_params = get_param_baseline(
            problem_type=self.problem_type, num_classes=self.num_classes
        )
    ...
    def _fit(
        ...
        **kwargs,
        ...
        logger.log(
            15, f"Training Gradient Boosting Model for {num_boost_round} rounds..."
        )
        ...
        if "min_data_in_leaf" in params:
            if (
                params["min_data_in_leaf"] > num_rows_train
        ...
        if dataset_val is not None:
            ...
            train_loss_name = (
                self._get_train_loss_name() if reporter is not None else None
            )
        ...
        if not isinstance(stopping_metric, str):
            ...
            if (
                "metric" not in train_params["params"]
                or train_params["params"]["metric"] == ""
            ):
                train_params["params"]["metric"] = stopping_metric
            elif stopping_metric not in train_params["params"]["metric"]:
                train_params["params"][
                    "metric"
                ] = f'{train_params["params"]["metric"]},{stopping_metric}'
        ...
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore", message="Overriding the parameters from Reference Dataset."
            )
            warnings.filterwarnings(
                "ignore", message="categorical_column in param dict is overridden."
            )
            try:
            ...
            if train_params["params"].get("boosting_type", "") == "dart":
                if (
                    ...
                    and dart_retrain
                    and (self.model.best_iteration != num_boost_round)
                ):
                    retrain = True
                    ...
                    if retrain:
                        ...
                        logger.log(
                            15, f"Not enough time to retrain LGB model ('dart' mode)..."
                        )
    ...
    def _predict_proba(self, X, **kwargs):
        ...
        if self.problem_type == BINARY:
            ...
            y_pred_proba = np.multiply(
                y_pred_proba, 1 / np.sum(y_pred_proba, axis=1)[:, np.newaxis]
            )
            return y_pred_proba
        else:
            if len(y_pred_proba.shape) == 1:
            ...
            for column in X.columns:
                if isinstance(column, str):
                    ...
                    if new_column != column:
                        self._internal_feature_map = {
                            feature: i for i, feature in enumerate(list(X.columns))
                        }
                        break

        if self._internal_feature_map:
            new_columns = [
                self._internal_feature_map[column] for column in list(X.columns)
            ]
    ...
    def generate_datasets(
        ...
        lgb_dataset_params_keys = [
            "objective",
            "two_round",
            "num_threads",
            "num_classes",
            "verbose",
        ]  # Keys that are specific to lightGBM Dataset object construction.
        data_params = {
            key: params[key] for key in lgb_dataset_params_keys if key in params
        }.copy()
        ...
        if self.problem_type == SOFTCLASS:
            if (not dataset_train) and (X is not None) and (y is not None):
                ...
                y = pd.Series(
                    [0] * len(X)
                )  # placeholder dummy labels to satisfy lgb.Dataset constructor
        ...
        if not dataset_train:
            ...
            dataset_train = construct_dataset(
                x=X,
                y=y,
                location=f"{self.path}datasets{os.path.sep}train",
                params=data_params,
                save=save,
    ...
    def debug_features_to_use(self, X_val_in):
        ...
        feature_importances_unused = feature_importances[
            feature_importances["splits"] == 0
        ]
        feature_importances_used = feature_importances[
            feature_importances["splits"] >= (total_splits / feature_count)
        ]
    ...
    def _hyperparameter_tune(self, X, y, X_val, y_val, scheduler_options, **kwargs):
        ...
        if isinstance(params_copy["min_data_in_leaf"], Int):
            ...
            if (
                upper_minleaf > X.shape[0]
            ):  # TODO: this min_data_in_leaf adjustment based on sample size may not be necessary
                ...
                params_copy["min_data_in_leaf"] = Int(
                    lower=lower_minleaf, upper=upper_minleaf
                )
        ...
        if ("dist_ip_addrs" in scheduler_params) and (
            ...
            scheduler.upload_files(
                [train_file, val_file, val_pkl_path]
    ...
    def get_model_feature_importance(self, use_original_feature_names=False):
        ...
        if use_original_feature_names and (self._internal_feature_map is not None):
            inverse_internal_feature_map = {
                i: feature for feature, i in self._internal_feature_map.items()
            }
            importance_dict = {
                inverse_internal_feature_map[i]: importance
                for i, importance in importance_dict.items()
            }
