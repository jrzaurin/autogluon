...
def test_advanced_functionality():
    ...
    assert set(feature_importances.columns) == {
        "importance",
        "stddev",
        "p_value",
        "n",
    ...
    assert set(predictor.get_model_names_persisted()) == set(
        persisted_models
    )  # Ensure all models are persisted
    assert (
    ...
    assert (
        predictor.unpersist_models(models=["UNKNOWN_MODEL_1", "UNKNOWN_MODEL_2"]) == []
    )
    ...
    predictor.delete_models(
        models_to_keep=[]
    )  # Test that dry-run doesn't delete models
    assert len(predictor.get_model_names()) == num_models * 2
    predictor.predict(data=test_data)
    predictor.delete_models(
        models_to_keep=[], dry_run=False
    )  # Test that dry-run deletes models
...
def run_tabular_benchmark_toy(fit_args):
    ...
    predictor = TabularPredictor(label=dataset["label"], path=savedir).fit(
        train_data, **fit_args
    )
...
def run_tabular_benchmarks(
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        for idx in range(len(datasets)):
            ...
            print(
                "Evaluating Benchmark Dataset %s (%d of %d)"
            ...
            if (not fast_benchmark) and (
                performance_vals[idx] > dataset["performance_val"] * perf_threshold
            ):
                warnings.warn(
                    "Performance on dataset %s is %s times worse than previous performance."
                    % (
                        dataset["name"],
                        performance_vals[idx] / (EPS + dataset["performance_val"]),
                    )
                )
            if predictor._trainer.bagged_mode:
                ...
                y_pred_proba_oof_transformed = predictor.get_oof_pred_proba(
                    transformed=True
                )
                ...
                if predictor.problem_type == MULTICLASS:
                    ...
                    if predictor.problem_type == BINARY:
                        assert isinstance(
                            predictor.get_oof_pred_proba(as_multiclass=True),
                            pd.DataFrame,
                        )
                    assert isinstance(y_pred_proba_oof, pd.Series)
                    assert isinstance(y_pred_proba_oof_transformed, pd.Series)

                assert y_pred_oof_transformed.equals(
                    predictor.transform_labels(y_pred_oof, proba=False)
                )
                ...
                y_pred_proba_oof_inverse = predictor.transform_labels(
                    y_pred_proba_oof, proba=True
                )
                ...
                y_pred_oof_inverse_inverse = predictor.transform_labels(
                    y_pred_oof_inverse, inverse=True
                )

                if isinstance(y_pred_proba_oof_transformed, pd.DataFrame):
                    pd.testing.assert_frame_equal(
                        y_pred_proba_oof_transformed, y_pred_proba_oof_inverse
                    )
                    pd.testing.assert_frame_equal(
                        y_pred_proba_oof, y_pred_proba_oof_inverse_inverse
                    )
                else:
                ...
                pd.testing.assert_series_equal(
                    y_pred_oof_transformed, y_pred_oof_inverse
                )
...
def test_tabularHPObagstack():
    ...
    subsample_size = None
    hyperparameter_tune_kwargs = {"searcher": "local_sequential_auto"}
    num_stack_levels = 2
    num_bag_folds = 2
    verbosity = 2  # how much output to print
    hyperparameters = None
