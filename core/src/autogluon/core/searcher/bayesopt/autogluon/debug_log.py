...
__all__ = ["ConfigCounter", "DebugLogPrinter"]
...
def _to_key(
    ...
    config_tpl = tuple(v for _, v in sorted(config_dict.items(), key=lambda x: x[0]))
...
class ConfigCounter(object):
    ...
    def __init__(self, configspace_ext: Optional[ExtendedConfiguration] = None):
    ...
    def add_config(self, config: Union[CS.Configuration, dict]) -> (dict, int):
        config_key, config_dct, resource = _to_key(config, self.configspace_ext)
    ...
    def get_mutable_state(self) -> dict:
        return {"config_counter": self.config_counter, "config_id": self._config_id}
...
class DebugLogPrinter(object):
    ...
    def start_get_config(self, gc_type):
        ...
        logger.info(
            "Starting get_config[{}] for config_id {}".format(
    ...
    def set_final_config(self, config: Union[CS.Configuration, dict]):
        ...
        assert resource is None, "set_final_config: config must not be extended"
    ...
    def set_state(self, state: TuningJobState):
        ...
        labeled_configs = [x.candidate for x in state.candidate_evaluations]
        labeled_str = ", ".join([self.config_id(x) for x in labeled_configs])
        pending_str = ", ".join([self.config_id(x) for x in state.pending_candidates])
    ...
    def set_init_config(
        self, config: Union[CS.Configuration, dict], top_scores: np.ndarray = None
    ):
        assert self.get_config_type == "BO", "Need to be in 'BO' block"
