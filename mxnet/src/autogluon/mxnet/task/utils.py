...
def get_data_loader(
    dataset, input_size, batch_size, num_workers, final_fit, split_ratio
):
    if isinstance(dataset, AutoGluonObject):
        dataset = dataset.init()
    elif isinstance(dataset, str):
        dataset = get_built_in_dataset(
            dataset,
            train=True,
...
def default_train_fn(
    ...
    with mx.autograd.record():
        ...
        if distillation:
            ...
            loss = [
                criterion(yhat, y.astype(dtype, copy=False))
...
def _train_val_split(train_dataset, split_ratio=0.2):
    ...
    return SampledDataset(train_dataset, train_sampler), SampledDataset(
        train_dataset, val_sampler
    )
