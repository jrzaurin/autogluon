...
def negative_log_posterior(likelihood: MarginalLikelihood, X: np.array, Y: np.array):
...
class GaussianProcessRegression(GaussianProcessModel):
    ...
    def _create_lbfgs_arguments(self, X, Y):
        ...
        """
        param_dict = {
            param.name: param for param in self.likelihood.collect_params().values()
        }
        ...
        def executor(param_vec):
            ...
            if self._debug_writer is not None:
                self._debug_writer.store_args(
                    self.likelihood.collect_params().values(),
                    X,
                    Y,
