        ...
        got = _pick_from_locally_optimized(
            candidates_with_optimization=[
                ...
                (
                    (0.2, 1.0),
                    (0.1, 1.0),
                ),  # duplicate optimized; Resolved by the original
...
def test_lazily_locally_optimize():
    original_candidates = [(1.0, "a", 3, "b"), (2.0, "c", 2, "a"), (0.0, "d", 0, "d")]
    ...
    assert (
        len(
            list(
                _lazily_locally_optimize(
                    [], NoOptimization(None, None, None), model=None
                )
            )
        )
        == 0
...
@pytest.mark.parametrize(
    ...
    [
        (
            {(1.0, "a"), (2.0, "a"), (1.0, "b"), (3.0, "a"), (2.0, "b"), (3.0, "b")},
            (1.0, 1.0, 2.0, 2.0, 3.0, 3.0),
        ),
        (
            {(1.0, "a", "b"), (1.0, "c", "b"), (3.0, "a", "d"), (0.0, "e", "b")},
            (0.0, 1.0, 1.0, 3.0),
        ),
        (
            {
...
def test_order_candidates(example, expected):
    ...
    class MyScorer(ScoringFunction):
        def score(
            self,
            candidates: Iterable[StateIdAndCandidate],
