...
def save(
    ...
    if type == "csv":
        if is_local:
            ...
            s3_resource.Object(bucket, prefix).put(
                Body=buffer.getvalue(), ACL="bucket-owner-full-control"
            )
        if verbose:
            logger.log(
                15,
        ...
        bucket, prefix = s3_utils.s3_path_to_bucket_prefix(s3_path=path)
        s3_utils.delete_s3_prefix(
            bucket=bucket, prefix=prefix
        )  # TODO: Might only delete the first 1000!
        save_multipart(
            path=path,
            df=df,
            index=index,
...
def save_multipart(
    ...
    logger.log(
        15, "Save_multipart running pool with " + str(workers_count) + " workers"
    )

    paths = [
        path + "part-" + "0" * (5 - min(5, len(str(i)))) + str(i) + ".parquet"
