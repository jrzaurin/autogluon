...
warnings.filterwarnings(
    "ignore", message=".*objective has been evaluated at this point before.*"
)
warnings.filterwarnings(
    "ignore",
    message=".*skopt failed to produce new config, using random search instead.*",
)
...
def test_skoptsearcher():
    ...
    a = CSH.UniformFloatHyperparameter(
        "a", lower=1e-4, upper=1e-1, log=True
    )  # log-scale float
    b = CSH.UniformFloatHyperparameter(
        "b", lower=-2, upper=0
    )  # float with uniform prior
    c = CSH.UniformIntegerHyperparameter("c", lower=0, upper=1000)  # integer
    d = CSH.CategoricalHyperparameter(
        "d", choices=["good", "neutral", "bad"]
    )  # categorical
    ...
    skopt_searcher = SKoptSearcher(cs, reward_attribute=reward_attribute)
    ...
    skrf_searcher = SKoptSearcher(
        cs, reward_attribute=reward_attribute, base_estimator="RF", acq_func="EI"
    )
    ...
    # Summarize results:
    logger.debug(
        "best reward from SKopt: %f,  best reward from SKopt w/ RF: %f,  best reward from Random search: %f"
        % (max(skopt_reward_list), max(skrf_reward_list), max(random_reward_list))
    )
    # Ensure skopt outperformed random search:
    assert max(skopt_reward_list) + 0.05 >= max(
        random_reward_list
    ), "SKopt did significantly worse than Random Search"
