...
class GreedyWeightedEnsembleModel(AbstractModel):
    def __init__(self, base_model_names=None, model_base=EnsembleSelection, **kwargs):
        ...
        self.num_pred_cols_per_model = (
            self.num_classes if self.problem_type in [MULTICLASS, SOFTCLASS] else 1
        )
        self.base_model_names = base_model_names
        if self.base_model_names is not None:
            self.features = self._set_stack_columns(
    ...
    def _fit(
        ...
        **kwargs
        ...
        self.model = self.model.fit(
            X, y, time_limit=time_limit, sample_weight=sample_weight
        )
    ...
    def _infer_base_model_names(self):
        stack_column_names = self.feature_metadata.get_features(
            required_special_types=[S_STACK]
        )

        if self.num_pred_cols_per_model > 1:
            columns_class_0 = [
                column for column in stack_column_names if column.endswith("_0")
            ]
    ...
    def _get_model_weights(self):
        ...
        model_weight_dict = {
            self.base_model_names[i]: self.weights_[i] for i in range(num_models)
        }
