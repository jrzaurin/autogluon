...
@pytest.mark.parametrize(
    "lower,upper,external_hp,internal_ndarray,scaling",
    [
...
def test_continuous_to_and_from_ndarray(
    ...
    assert_allclose(hp_range.to_ndarray(external_hp), np.array([internal_ndarray]))
    assert_allclose(hp_range.from_ndarray(np.array([internal_ndarray])), external_hp)
...
def test_integer_to_and_from_ndarray(
    lower, upper, external_hp, internal_ndarray, scaling
):
...
def _test_continuous_to_ndarray_and_back(lower, upper, external_hp, scaling):
    hp_range = HyperparameterRangeContinuous("hp", lower, upper, scaling)
    assert hp_range.from_ndarray(hp_range.to_ndarray(external_hp)) == approx(
        external_hp
    )
...
def test_distribution_of_random_candidates():
    ...
    random_candidates = [
        hp_ranges.random_candidate(random_state) for _ in range(num_random_candidates)
    ]
...
@pytest.mark.parametrize(
    "lower,upper,scaling,constructor",
    [
