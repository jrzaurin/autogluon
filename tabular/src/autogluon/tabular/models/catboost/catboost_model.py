        ...
        if self.problem_type != SOFTCLASS:  # TODO: remove this after catboost 0.24
            self._set_default_param_value(
                ...
                construct_custom_catboost_metric(
                    ...
                    True,
                    not self.stopping_metric.needs_pred,
                    self.problem_type,
                ),
            )
        ...
        if self._category_features:
            ...
            for category in self._category_features:
                ...
                if "__NaN__" in current_categories:
                    ...
                    X[category] = (
                        X[category].cat.add_categories("__NaN__").fillna("__NaN__")
                    )
        return X
    ...
    def _fit(
        ...
        **kwargs,
        ...
        if self.problem_type == SOFTCLASS:
            ...
            from .catboost_softclass_utils import (
                SoftclassCustomMetric,
                SoftclassObjective,
            )
        ...
        if self.problem_type == MULTICLASS:
            ...
            self.problem_type == SOFTCLASS
        ):  # TODO: delete this elif if it's unnecessary.
        ...
        approx_mem_size_req = (
            num_rows_train * num_cols_train * num_classes / 2
        ...
        if approx_mem_size_req > 1e9:  # > 1 GB
            ...
            if ratio > (1 * max_memory_usage_ratio):
                logger.warning(
                    ...
                    % (
                        round(approx_mem_size_req / 1e9, 3),
                        round(available_mem / 1e9, 3),
                    )
                )
        ...
        if time_limit:
            ...
            if self.stopping_metric._optimum == init_model_best_score:
                ...
                if (
                    self.problem_type == SOFTCLASS
                ):  # TODO: remove this once catboost-dev is no longer necessary and SOFTCLASS objectives can be pickled.
                ...
                params_final["iterations"] = min(
                    iterations_left, estimated_iters_in_time
                )
        ...
        if params_final is not None and params_final["iterations"] > 0:
            ...
            if init_model is not None:
                ...
                if self.stopping_metric._optimum == init_model_best_score:
                    ...
                    if (init_model_best_score > self.stopping_metric._optimum) or (
                        final_model_best_score > self.stopping_metric._optimum
                    ):
                    ...
                    if warm_start:
                        if init_model_best_score >= final_model_best_score:
                            ...
                            best_iteration = (
                                init_model_tree_count + self.model.get_best_iteration()
                            )
                            self.model.shrink(
                                ntree_start=0, ntree_end=best_iteration + 1
                            )
                    else:
                        if init_model_best_score >= final_model_best_score:
                            self.model = init_model
