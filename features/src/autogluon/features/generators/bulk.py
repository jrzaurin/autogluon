    ...
    **kwargs :
        ...
        **kwargs,
        ...
        if not isinstance(generators, list):
            ...
            raise AssertionError(
                "generators must contain at least one AbstractFeatureGenerator."
            )
        generators = [
        ...
        if self._post_generators is not None:
            post_generators = [
                [post_generator] for post_generator in self._post_generators
            ]
            self._post_generators = []
        else:
            post_generators = []
        self.generators: List[List[AbstractFeatureGenerator]] = (
            pre_generators + generators + post_generators
        )
        ...
        for i in range(len(self.generators)):
            ...
            for generator in self.generators[i]:
                if generator.is_valid_metadata_in(feature_metadata):
                    ...
                    generator.set_log_prefix(
                        log_prefix=self.log_prefix + "\t\t", prepend=True
                    )
                    ...
                    self._log(
                        15,
                        f"\t\tSkipping {generator.__class__.__name__}: No input feature with required dtypes.",
                    )
            ...
            self.generators[i] = [
                ...
                if feature_df_list[j] is not None
                and len(feature_df_list[j].columns) > 0
            ]
            feature_df_list = [
                feature_df
            ...
            if self.generators[i]:
                feature_metadata = FeatureMetadata.join_metadatas(
                    [generator.feature_metadata for generator in self.generators[i]],
    ...
    def _remove_unused_features(self, feature_links_chain):
        ...
        if unused_features_by_stage:
            ...
            feature_metadata_in_unused = self.feature_metadata_in.keep_features(
                features=unused_features_in
            )
            if self._feature_metadata_in_unused:
                self._feature_metadata_in_unused = (
                    self._feature_metadata_in_unused.join_metadata(
        ...
        for i, generator_group in enumerate(self.generators):
            ...
            for generator in generator_group:
                unused_features_out_generator = [
                    ...
                    if feature in unused_features_out_stage
                ]
            ...
            if stage > 1:
                if self.generators[stage - 2]:
                    features_in = FeatureMetadata.join_metadatas(
                        [
                            generator.feature_metadata
                            for generator in self.generators[stage - 2]
                        ],
                        shared_raw_features="error",
                    ).get_features()
                else:
