...
class AbstractTrainer:
    ...
    def __init__(
        ...
        self.feature_prune = (
            False  # will be set to True if feature-pruning is turned on.
        )
        self.low_memory = low_memory
        self.bagged_mode = True if k_fold >= 2 else False
        if self.bagged_mode:
            self.k_fold = (
                k_fold  # int number of folds to do model bagging, < 2 means disabled
            )
        ...
        self.model_full_dict = (
            {}
        )  # Dict of normal model -> FULL model. FULL models are produced by self.refit_single_full() and self.refit_ensemble_full().
    ...
    def get_model_names(
        self,
        stack_name: Union[List[str], str] = None,
        level: Union[List[int], int] = None,
        can_infer: bool = None,
        ...
        if stack_name is not None:
            ...
            node_attributes: dict = self.get_models_attribute_dict(
                attribute="stack_name"
            )
            models = [
                model_name
                for model_name in models
                if node_attributes[model_name] in stack_name
            ]
        if level is not None:
            ...
            models = [
                model_name
                for model_name in models
                if node_attributes[model_name] in level
            ]
    ...
    def get_max_level(self, stack_name: str = None, models: List[str] = None) -> int:
        ...
        models_attribute_dict = self.get_models_attribute_dict(
        ...
        for level in range(level_start, level_end + 1):
            ...
            if time_limit is not None:
                ...
                time_limit_for_level = (
                    time_limit - (time_train_level_start - time_train_start)
                ) / (level_end + 1 - level)
                ...
                core_kwargs_level["time_limit"] = core_kwargs_level.get(
                    "time_limit", time_limit_core
                )
                aux_kwargs_level["time_limit"] = aux_kwargs_level.get(
                    "time_limit", time_limit_aux
                )
            if level != 1:
    ...
    def stack_new_level(
        ...
        if level < 1:
            ...
            logger.log(
                30,
                f"Warning: Training models at stack level {level}, but no base models were specified.",
            )
        ...
        aux_kwargs = {} if aux_kwargs is None else aux_kwargs.copy()
        if name_suffix:
            core_kwargs["name_suffix"] = (
                core_kwargs.get("name_suffix", "") + name_suffix
            )
        ...
        if self.bagged_mode:
            aux_models = self.stack_new_level_aux(
                X=X, y=y, base_model_names=core_models, level=level + 1, **aux_kwargs
            )
        else:
            aux_models = self.stack_new_level_aux(
                X=X_val,
    ...
    def stack_new_level_core(
        ...
        **kwargs,
        ...
        if isinstance(models, dict):
            ...
            if self.bagged_mode:
                if level == 1:
                    ...
                    (
                        ...
                        base_model_paths,
                        base_model_types,
                    ) = self._get_models_load_info(model_names=base_model_names)
                    if len(base_model_names) == 0:
                        logger.log(
                            20, "No base models to train on, skipping stack level..."
                        )
                        return []
                else:
                    raise AssertionError(
                        f"Stack level cannot be less than 1! level = {level}"
                    )
            ...
            models, model_args_fit = get_models_func(
                hyperparameters=models, **get_models_kwargs
            )
            if model_args_fit:
        ...
        if X_val is not None:
            X_val = self.get_inputs_to_stacker(
                X_val, base_models=base_model_names, fit=False
            )
        if X_unlabeled is not None:
            X_unlabeled = self.get_inputs_to_stacker(
                X_unlabeled, base_models=base_model_names, fit=False
            )
    ...
    def stack_new_level_aux(
        ...
        X_stack_preds = self.get_inputs_to_stacker(
            X, base_models=base_model_names, fit=fit, use_orig_features=False
        )
    ...
    def get_model_pred_proba_dict(
        ...
        if fit:
            model_pred_order = [
                model for model in models if model not in model_pred_proba_dict.keys()
            ]
        else:
            model_set = set()
            for model in models:
            ...
            models_to_ignore = [
                ...
                if (model not in models) and (not list(subgraph.successors(model)))
            ]
        ...
        for model_name in model_pred_order:
            ...
            if fit:
                ...
                if issubclass(model_type, BaggedEnsembleModel):
                    model_path = self.get_model_attribute(
                        model=model_name, attribute="path"
                    )
                    model_pred_proba_dict[model_name] = model_type.load_oof(
                        path=model_path
                    )
                else:
                    raise AssertionError(
                        f"Model {model_name} must be a BaggedEnsembleModel to return oof_pred_proba"
                    )
            else:
                model = self.load_model(model_name=model_name)
                if isinstance(model, StackerEnsembleModel):
                    preprocess_kwargs = dict(
                        infer=False, model_pred_proba_dict=model_pred_proba_dict
                    )
                    model_pred_proba_dict[model_name] = model.predict_proba(
                        X, **preprocess_kwargs
                    )
                else:
    ...
    def get_inputs_to_stacker(
        ...
        if not fit:
            model_pred_proba_dict = self.get_model_pred_proba_dict(
                X=X, models=base_models, model_pred_proba_dict=model_pred_proba_dict
            )
            model_pred_proba_list = [
                model_pred_proba_dict[model] for model in base_models
            ]
        else:
        ...
        X_stacker_input = self._get_inputs_to_stacker_legacy(
            X=X,
            level_start=1,
            level_end=2,
            model_levels={1: base_models},
            y_pred_probas=model_pred_proba_list,
            fit=fit,
        )
    ...
    def _get_inputs_to_stacker_legacy(
        self, X, level_start, level_end, model_levels, y_pred_probas=None, fit=False
    ):
        if level_start > level_end:
            raise AssertionError(
                f"level_start cannot be greater than level end: ({level_start}, {level_end})"
            )
        if (level_start == 1) and (level_end == 1):
            return X
        if fit:
            if level_start > 1:
                dummy_stacker_start = self._get_dummy_stacker(
                    level=level_start, model_levels=model_levels, use_orig_features=True
                )
                cols_to_drop = dummy_stacker_start.stack_columns
                X = X.drop(cols_to_drop, axis=1)
            dummy_stacker = self._get_dummy_stacker(
                level=level_end, model_levels=model_levels, use_orig_features=True
            )
            X = dummy_stacker.preprocess(
                X=X, preprocess_nonadaptive=False, fit=True, compute_base_preds=True
            )
            ...
            X_stacker = dummy_stacker.pred_probas_to_df(
                pred_proba=y_pred_probas, index=X.index
            )
            ...
            for level in range(level_start, level_end + 1):
                if level > 1:
                    dummy_stackers[level] = self._get_dummy_stacker(
            ...
            for level in range(level_start, level_end):
                if level > 1:
                    ...
                    cols_to_drop = []
                X = dummy_stackers[level + 1].preprocess(
                    X=X,
                    preprocess_nonadaptive=False,
                    fit=False,
    ...
    def refit_single_full(
        self, X=None, y=None, X_val=None, y_val=None, X_unlabeled=None, models=None
    ) -> List[str]:
        if X is None:
            X = self.load_X()
        ...
        for level in levels:
            ...
            for model in models_level:
                ...
                model_full.params_aux["max_memory_usage_ratio"] = (
                    model_full.params_aux["max_memory_usage_ratio"] * 1.15
                )
                ...
                if issubclass(stacker_type, WeightedEnsembleModel):
                    ...
                    if self.bagged_mode:
                        ...
                        X_stack_preds = self.get_inputs_to_stacker(
                            ...
                            fit=False,
                            use_orig_features=False,
                        )  # TODO: May want to cache this during original fit, as we do with OOF preds
                        y_input = y_val
                    if w is not None:
                        X_stack_preds[self.sample_weight] = w.values / w.mean()
                    ...
                    child_hyperparameters[AG_ARGS_FIT] = copy.deepcopy(
                        model_child.params_aux
                    )
                    ...
                    for model_weighted_ensemble in models_trained:
                        ...
                        self.set_model_attribute(
                            model=model_weighted_ensemble,
                            attribute="val_score",
                            val=None,
                        )
        ...
        for model_full in models_trained_full:
            ...
            if isinstance(model_loaded, StackerEnsembleModel):
                for stack_column_prefix in model_loaded.stack_column_prefix_lst:
                    base_model = model_loaded.stack_column_prefix_to_model_map[
                        stack_column_prefix
                    ]
                    new_base_model = self.model_full_dict[base_model]
                    new_base_model_type = self.get_model_attribute(
                        model=new_base_model, attribute="type"
                    )
                    new_base_model_path = self.get_model_attribute(
                        model=new_base_model, attribute="path"
                    )

                    model_loaded.base_model_paths_dict[
                        new_base_model
                    ] = new_base_model_path
                    model_loaded.base_model_types_dict[
                        new_base_model
                    ] = new_base_model_type
                    model_loaded.base_model_names.append(new_base_model)
                    model_loaded.stack_column_prefix_to_model_map[
                        stack_column_prefix
                    ] = new_base_model
            ...
            if isinstance(model_loaded, StackerEnsembleModel):
                for stack_column_prefix in model_loaded.stack_column_prefix_lst:
                    base_model_name = model_loaded.stack_column_prefix_to_model_map[
                        stack_column_prefix
                    ]
    ...
    def get_model_best(self, can_infer=None, allow_full=True):
        ...
        if not perfs:
            model_full_dict_inverse = {
                full: orig for orig, full in self.model_full_dict.items()
            }
    ...
    def persist_models(
        self, model_names="all", with_ancestors=False, max_memory=None
    ) -> List[str]:
        if model_names == "all":
        ...
        if not isinstance(model_names, list):
            raise ValueError(
                f"model_names must be a list of model names. Invalid value: {model_names}"
            )
        if with_ancestors:
            model_names = self.get_minimum_models_set(model_names)
        model_names_already_persisted = [
            model_name for model_name in model_names if model_name in self.models
        ]
        ...
        model_names = [
            ...
            if model_name not in model_names_already_persisted
        ]
        if not model_names:
            logger.log(
                30,
        ...
        if max_memory is not None:
            ...
            model_mem_size_map = {
                model: info[model]["memory_size"] for model in model_names
            }
    ...
    def unpersist_models(self, model_names="all") -> list:
        ...
        if unpersisted_models:
            logger.log(
                20,
                f"Unpersisted {len(unpersisted_models)} models: {unpersisted_models}",
            )
        else:
            logger.log(
                30,
    ...
    def generate_weighted_ensemble(
        ...
        if save_bag_folds is None:
            can_infer_dict = self.get_models_attribute_dict(
                "can_infer", models=base_model_names
            )
        ...
        weighted_ensemble_model, _ = get_models_func(
            ...
            ensemble_kwargs=dict(
                base_model_names=base_model_names,
                base_model_paths_dict=self.get_models_attribute_dict(
                    attribute="path", models=base_model_names
                ),
                base_model_types_dict=self.get_models_attribute_dict(
                    attribute="type", models=base_model_names
                ),
        ...
        for weighted_ensemble_model_name in models:
            if check_if_best and weighted_ensemble_model_name in self.get_model_names():
                if self.model_best is None:
                    ...
                    cur_score = self.get_model_attribute(
                        weighted_ensemble_model_name, "val_score"
                    )
    ...
    def _train_single(
        self, X, y, model: AbstractModel, X_val=None, y_val=None, **model_fit_kwargs
    ) -> AbstractModel:
        """
        ...
        try:
            if time_limit is not None:
                ...
                if self._time_limit is not None and self._time_train_start is not None:
                    time_left_total = self._time_limit - (
                        fit_start_time - self._time_train_start
                    )
                else:
                    time_left_total = time_limit
                logger.log(
                    20,
                    f"Fitting model: {model.name} ... Training model for up to {round(time_limit, 2)}s of the {round(time_left_total, 2)}s of remaining time.",
                )
            else:
            ...
            logger.warning(
                f"\tNot enough memory to train {model.name}... Skipping this model."
            )
            del model
        except NoValidFeatures:
            logger.warning(
                f"\tNo valid features to train {model.name}... Skipping this model."
            )
            del model
        except NoGPUError:
            logger.warning(
                f"\tNo GPUs available to train {model.name}... Skipping this model."
            )
            del model
    ...
    def _add_model(
        self, model: AbstractModel, stack_name: str = "core", level: int = 1
    ) -> bool:
        """
        ...
        if model.val_score is not None:
            ...
            logger.log(
                20,
                f"\t{round(model.val_score, 4)}\t = Validation {model.eval_metric.name} score",
            )
        ...
        self.model_graph.add_node(
            ...
            type=type(
                model
            ),  # Outer type, can be BaggedEnsemble, StackEnsemble (Type that is able to load the model)
        ...
        if isinstance(model, StackerEnsembleModel):
            ...
            for stack_column_prefix in model.stack_column_prefix_lst:
                base_model_name = model.stack_column_prefix_to_model_map[
                    stack_column_prefix
                ]
    ...
    def _train_single_full(
        ...
        **kwargs,
        ...
        if hyperparameter_tune_kwargs:
            if n_repeat_start != 0:
                raise ValueError(
                    f"n_repeat_start must be 0 to hyperparameter_tune, value = {n_repeat_start}"
                )
            elif k_fold_start != 0:
                raise ValueError(
                    f"k_fold_start must be 0 to hyperparameter_tune, value = {k_fold_start}"
                )
            ...
            try:
                ...
                for model_hpo_name, model_path in hpo_models.items():
                    ...
                    if self._add_model(
                        model=model_hpo, stack_name=stack_name, level=level
                    ):
        ...
        **kwargs,
        ...
        for n in range(n_repeat_start, n_repeats):
            if time_limit is not None:
                ...
                if n == n_repeat_start:
                    time_required = (
                        time_limit_total_level * 0.575
                    )  # Require slightly over 50% to be safe
                else:
                    time_required = (
                        (time_start_repeat - time_start)
                        / repeats_completed
                        * (0.575 / 0.425)
                    )
        ...
        logger.log(
            20,
            f"Completed {n_repeat_start + repeats_completed}/{n_repeats} k-fold bagging repeats ...",
        )
        return models_valid
    ...
    def _train_multi_fold(
        ...
        **kwargs,
        ...
        for i, model in enumerate(models):
            ...
            if hyperparameter_tune_kwargs is not None and isinstance(
                hyperparameter_tune_kwargs, dict
            ):
                hyperparameter_tune_kwargs_model = hyperparameter_tune_kwargs.get(
                    model.name, None
                )
            else:
    ...
    def _train_multi(
        ...
        **kwargs,
        ...
        if (k_fold == 0) and (n_repeats != 1):
            raise ValueError(
                f"n_repeats must be 1 when k_fold is 0, values: ({n_repeats}, {k_fold})"
            )
        if time_limit is None:
    ...
    def _predict_model(self, X, model, model_pred_proba_dict=None):
        if isinstance(model, str):
            model = self.load_model(model)
        X = self.get_inputs_to_model(
            model=model, X=X, model_pred_proba_dict=model_pred_proba_dict, fit=False
        )
        y_pred = model.predict(X=X)
        if (
            self._regress_preds_asprobas and model.problem_type == REGRESSION
        ):  # Convert regression preds to classes (during distillation)
    ...
    def _get_dummy_stacker(
        self, level: int, model_levels: dict, use_orig_features=True
    ) -> StackerEnsembleModel:
        ...
        dummy_stacker = StackerEnsembleModel(
            ...
            base_model_paths_dict=self.get_models_attribute_dict(
                attribute="path", models=model_names
            ),
            base_model_types_dict=self.get_models_attribute_dict(
                attribute="type", models=model_names
            ),
            hyperparameters=hyperparameters,
    ...
    def get_feature_importance(
        self, model=None, X=None, y=None, raw=True, **kwargs
    ) -> pd.DataFrame:
        if model is None:
    ...
    def _get_feature_importance_raw(
        self, X, y, model, eval_metric=None, **kwargs
    ) -> pd.DataFrame:
    ...
    def _get_models_load_info(self, model_names):
        ...
        model_paths = self.get_models_attribute_dict(
            attribute="path", models=model_names
        )
        model_types = self.get_models_attribute_dict(
            attribute="type", models=model_names
        )
    ...
    def get_models_attribute_dict(self, attribute, models: list = None) -> dict:
        ...
        if models is not None:
            ...
            models_attribute_dict = {
                key: val
                for key, val in models_attribute_dict.items()
    ...
    def leaderboard(self, extra_info=False):
        ...
        for model_name in model_names:
            ...
            fit_time.append(
                self.get_model_attribute_full(model=model_name, attribute="fit_time")
            )
            pred_time_val_marginal.append(predict_time_marginal_dict[model_name])
            pred_time_val.append(
                self.get_model_attribute_full(
                    model=model_name, attribute="predict_time"
                )
            )
        ...
        if extra_info:
            ...
            for model_name in model_info:
                ...
                custom_info["memory_size"] = bagged_info.get(
                    "max_memory_size", model_info[model_name]["memory_size"]
                )
                custom_info["memory_size_min"] = bagged_info.get(
                    "min_memory_size", model_info[model_name]["memory_size"]
                )
                custom_info["child_model_type"] = bagged_info.get(
                    "child_model_type", None
                )
                custom_info["child_hyperparameters"] = bagged_info.get(
                    "child_hyperparameters", None
                )
                custom_info["child_hyperparameters_fit"] = bagged_info.get(
                    "child_hyperparameters_fit", None
                )
                custom_info["child_ag_args_fit"] = bagged_info.get(
                    "child_ag_args_fit", None
                )
            ...
            for key in model_info_keys:
                model_info_dict[key] = [
                    model_info[model_name][key] for model_name in model_names
                ]
                if key in model_info_sum_keys:
                    key_dict = {
                        model_name: model_info[model_name][key]
                        for model_name in model_names
                    }
                    model_info_dict[key + "_full"] = [
                        self.get_model_attribute_full(
                            model=model_name, attribute=key_dict
                        )
                        for model_name in model_names
                    ]
            ...
            for key in model_info_keys:
                model_info_dict[key] = [
                    custom_model_info[model_name][key] for model_name in model_names
                ]
                if key in model_info_full_keys:
                    key_dict = {
                        model_name: custom_model_info[model_name][key]
                        for model_name in model_names
                    }
                    for column_name, func in model_info_full_keys[key]:
                        model_info_dict[column_name] = [
                            self.get_model_attribute_full(
                                model=model_name, attribute=key_dict, func=func
                            )
                            for model_name in model_names
                        ]

            ancestors = [
                list(nx.dag.ancestors(self.model_graph, model_name))
                for model_name in model_names
            ]
            descendants = [
                list(nx.dag.descendants(self.model_graph, model_name))
                for model_name in model_names
            ]

            model_info_dict["num_ancestors"] = [
                len(ancestor_lst) for ancestor_lst in ancestors
            ]
            model_info_dict["num_descendants"] = [
                len(descendant_lst) for descendant_lst in descendants
            ]
    ...
    def delete_models(
        ...
        if models_to_keep is not None and models_to_delete is not None:
            raise ValueError(
                "Exactly one of [models_to_keep, models_to_delete] must be set."
            )
        if models_to_keep is not None:
            ...
            models_to_remove = [
                ...
                if model not in minimum_model_set
            ]
            ...
            for model in models_to_delete:
                minimum_model_set.update(
                    nx.algorithms.dag.descendants(self.model_graph, model)
                )
            ...
            minimum_model_set = list(minimum_model_set)
            models_to_remove = [
                model for model in self.get_model_names() if model in minimum_model_set
            ]
        else:
            raise ValueError(
                "Exactly one of [models_to_keep, models_to_delete] must be set."
        ...
        if dry_run:
            logger.log(
                30,
                f"Dry run enabled, AutoGluon would have deleted the following models: {models_to_remove}",
            )
            if delete_from_disk:
    ...
    def _get_full_model_val_score(self, model: str) -> float:
        ...
        if normal_model not in model_performances:
            ...
            if model not in self._model_full_dict_val_score:
                raise ValueError(
                    f"_FULL model {model} had the model it was based on ({normal_model}) deleted, and the validation score was not stored."
                )
    ...
    def distill(
        ...
        if teacher_preds in ["onehot", "soft"]:
            ...
            y_val = format_distillation_labels(
                y_val, self.problem_type, self.num_classes
            )
        ...
        if augment_method is None and augmentation_data is None:
                ...
                if (self.problem_type != REGRESSION) and (
                    ...
                    for clss in y.unique():
                        if clss not in y_pred.unique():
                            logger.log(
                                15,
                                f"Fetching a row with label={clss} from training data",
                            )
            ...
            if len(X_aug) > 0:
                if teacher_preds == "hard":
                    ...
                    raise ValueError(
                        f"Unknown teacher_preds specified: {teacher_preds}"
                    )
                ...
                if self.sample_weight is not None:
                    w = pd.concat(
                        [w, pd.Series([augmented_sample_weight] * len(X_aug))]
                    )
        ...
        for (
            ...
            models
        ):  # finally measure original metric on validation data and overwrite stored val_scores
        ...
        leaderboard_distilled = leaderboard[
