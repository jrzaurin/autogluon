...
def generate_csv_submission(
    dataset_path, dataset, local_path, inds, preds, class_name, custom
):
    """
...
def generate_prob_csv(
    ...
    def ensemble_csv(glob_files):
        ...
        if scale_min_max:
            w = w.apply(
                lambda x: np.round((x - min(x)) / (1.0 * (max(x) - min(x))), 2), axis=1
            )
