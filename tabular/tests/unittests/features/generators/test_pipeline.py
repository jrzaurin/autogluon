...
def test_pipeline_feature_generator(generator_helper, data_helper):
    ...
    toy_vectorizer = CountVectorizer(
        min_df=2, ngram_range=(1, 3), max_features=10, dtype=np.uint8
    )
...
def test_pipeline_feature_generator_dummy(generator_helper, data_helper):
    ...
    generator = PipelineFeatureGenerator(
        generators=[
            [
                IdentityFeatureGenerator(
                    infer_features_in_args=dict(valid_raw_types=["unknown_raw_type"])
                ),
            ]
        ]
    )
    ...
    assert list(generator.transform(input_data_transform)["__dummy__"].values) == [
        0,
        0,
        0,
        0,
    ...
    assert list(
        generator.transform(input_data_transform.head(5))["__dummy__"].values
    ) == [0, 0, 0, 0, 0]
...
def test_pipeline_feature_generator_removal_advanced(generator_helper, data_helper):
    ...
    generator = PipelineFeatureGenerator(
        generators=[
            ...
            [
                IdentityFeatureGenerator(
                    infer_features_in_args=dict(valid_raw_types=[R_CATEGORY])
                )
            ],
        ]
    )
