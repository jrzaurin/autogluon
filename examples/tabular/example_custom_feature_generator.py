...
print(
    X_transform.head(5)
)  # Now the output only contains the int type features, acting as a type filter.
...
bulk_feature_generator = BulkFeatureGenerator(
    generators=[
        [
            AsTypeFeatureGenerator()
        ],  # Stage 1: Convert feature types to be the same as during fit.
        ...
        [
            DropUniqueFeatureGenerator()
        ],  # Stage 4: Drop any features which are always the same value (useless).
    ],
    verbosity=3,
)
...
print(X_transform.head(5))
X_test_transform = bulk_feature_generator.transform(
    X=X_test
)  # Can now transform the test data based on the fit data.
