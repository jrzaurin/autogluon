...
from .gp_fifo_searcher import (
    GET_CONFIG_RANDOM_RETRIES,
    accumulate_profiling_record,
    MapReward,
    check_initial_candidates_scorer,
...
from ..tuning_algorithms.common import (
    RandomStatefulCandidateGenerator,
    compute_blacklisted_candidates,
)
from ..tuning_algorithms.default_algorithm import (
...
class GPMultiFidelitySearcher(object):
    ...
    def update(self, config: CS.Configuration, reward: float, resource: int, **kwargs):
        """
        ...
        self.state_transformer.label_candidate(
            CandidateEvaluation(candidate=config_ext, metrics=metrics)
        )
    ...
    def _get_unique_candidates(
        self, candidate_list: Iterable[CS.Configuration], target_resource: int
    ...
    def _get_blacklisted_candidates(
        self, target_resource: int
    ) -> Set[CS.Configuration]:
        """
    ...
    def _fix_resource_attribute(self, resource_attr_value: int):
        self.configspace_ext.hp_ranges_ext.value_for_last_pos = resource_attr_value
    ...
    def get_config(self, **kwargs) -> CS.Configuration:
        ...
        state = self.state_transformer.state
        if self.do_profile:
            fit_hyperparams = not self.state_transformer.skip_optimization(state)
        ...
        pick_random = (
            len(blacklisted_candidates) < self.num_initial_random_choices
        ) or (not state.candidate_evaluations)
        if self.debug_log is not None:
        ...
        if pick_random:
            ...
            initial_candidates_scorer = create_initial_candidates_scorer(
                self.initial_scoring, model, self.acquisition_class, self.random_state
            )
    ...
    def cleanup_pending(self, config: CS.Configuration):
        ...
        def filter_pred(x: PendingEvaluation) -> bool:
            x_dct = self.configspace_ext.remove_resource(x.candidate, as_dict=True)
    ...
    def clone_from_state(self, state):
        ...
        init_state = decode_state(state["state"], self.configspace_ext.hp_ranges_ext)
        ...
        new_searcher.random_generator.random_state = state["random_state"]
...
def draw_random_candidate(
    ...
    target_resource: int,
) -> (CS.Configuration, CS.Configuration):
    ...
    for _ in range(GET_CONFIG_RANDOM_RETRIES):
        ...
        _config_ext = configspace_ext.remap_resource(_config, target_resource)
...
def _max_at_least_k(k, counter=None):
    ...
    return max(
        filter(lambda r: counter[r] >= k, counter.keys()), default=min(counter.keys())
    )
...
def resource_for_acquisition_bohb(threshold: int) -> Callable[..., int]:
    ...
    def rfa_map(state: TuningJobState, resource_attr_name: str, **kwargs) -> int:
        ...
        histogram = Counter(
            [
                int(x.candidate.get_dictionary()[resource_attr_name])
...
def resource_for_acquisition_first_milestone(
    state: TuningJobState, resource_attr_name: str, **kwargs
) -> int:
    """
