...
def sample_posterior_marginals(
    features, mean, kernel, chol_fact, pred_mat, test_features, num_samples=1
):
    """
    ...
    n01_vecs = [
        anp.random.normal(size=getval(post_means.shape)) for _ in range(num_samples)
    ]
...
def sample_posterior_joint(
    ...
    n01_vecs = [
        anp.random.normal(size=getval(posterior_mean.shape)) for _ in range(num_samples)
    ]
...
def sample_and_cholesky_update(
    features, chol_fact, pred_mat, mean, kernel, noise_variance, feature
):
