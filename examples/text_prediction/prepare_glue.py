...
_URL_FILE_STATS.update(
    load_checksum_stats(os.path.join(_CURR_DIR, "url_checksums", "superglue.txt"))
)
            ...
            for line in f:
                ...
                for qa in sample["passage"]["questions"]:
                    ...
                    for ans in qa["answers"]:
                        ...
                        if fold == "test":
                            out.append(
                                (passage, question, answer, psg_idx, qst_idx, ans_idx)
                            )
                        else:
                            label = ans["label"]
                            out.append(
...
def get_parser():
    ...
    parser.add_argument(
        "--benchmark", choices=["glue", "superglue"], default="glue", type=str
    )
    parser.add_argument(
        "-d", "--data_dir", help="directory to save data to", type=str, default=None
    )
    parser.add_argument(
        "-t",
        "--tasks",
...
def main(args):
    ...
    print(
        "Downloading {} to {}. Selected tasks = {}".format(
        ...
        if task == "diagnostic" or "diagnostic" in task:
            if args.benchmark == "glue":
                ...
                for key, name in [
                    ...
                    df.to_parquet(
                        os.path.join(args.data_dir, name, "{}.parquet".format(name))
                    )
        elif task == "mrpc":
            reader = TASK2READER[task]
            ...
            for key, df in df_dict.items():
                if key == "val":
                    key = "dev"
                df.to_parquet(
                    os.path.join(args.data_dir, "mrpc", "{}.parquet".format(key))
                )
