...
from sklearn.metrics import (
    accuracy_score,  # , roc_curve, auc; , max_error
...
class AbstractLearner:
    ...
    def predict_proba(
        ...
        if as_pandas:
            ...
            X_index = None
        y_pred_proba = self.load_trainer().predict_proba(
            self.transform_features(X), model=model
        )
        ...
        if as_pandas:
            if self.problem_type == MULTICLASS or (
                as_multiclass and self.problem_type == BINARY
            ):
                y_pred_proba = pd.DataFrame(
                    data=y_pred_proba, columns=self.class_labels, index=X_index
                )
            else:
                y_pred_proba = pd.Series(
                    data=y_pred_proba, name=self.label, index=X_index
                )
        return y_pred_proba
    ...
    def predict(self, X: DataFrame, model=None, as_pandas=False):
        ...
        y_pred = get_pred_from_proba(
            y_pred_proba=y_pred_proba, problem_type=problem_type
        )
    ...
    def _validate_sample_weight(self, X, X_val):
        if self.sample_weight is not None:
            ...
            if weight_vals.isna().sum() > 0:
                raise ValueError(
                    f"Sample weights in column '{self.sample_weight}' cannot be nan"
                )
            if weight_vals.dtype.kind not in "biuf":
                raise ValueError(
                    f"Sample weights in column '{self.sample_weight}' must be numeric values"
                )
            if weight_vals.min() < 0:
                raise ValueError(
                    f"Sample weights in column '{self.sample_weight}' must be nonnegative"
                )
            if (
                self.weight_evaluation
                and X_val is not None
                and self.sample_weight not in X_val.columns
            ):
                raise KeyError(
    ...
    def get_inputs_to_stacker(
        self, dataset=None, model=None, base_models: list = None, use_orig_features=True
    ):
        if model is not None or base_models is not None:
            if model is not None and base_models is not None:
                raise AssertionError(
                    "Only one of `model`, `base_models` is allowed to be set."
                )
    ...
    def fit_transform_features(self, X, y=None, **kwargs):
        ...
        if self.ignored_columns:
            logger.log(
                20, f"Dropping user-specified ignored columns: {self.ignored_columns}"
            )
    ...
    def score(self, X: DataFrame, y=None, model=None):
        ...
        return compute_weighted_metric(
            y, y_pred, self.eval_metric, w, weight_evaluation=self.weight_evaluation
        )

    # Scores both learner and all individual models, along with computing the optimal ensemble score + weights (oracle)
    def score_debug(
        self, X: DataFrame, y=None, extra_info=False, compute_oracle=False, silent=False
    ):
        ...
        ) = trainer.get_model_pred_proba_dict(
            X=X, models=all_trained_models_can_infer, fit=False, record_pred_time=True
        )
        ...
        if compute_oracle:
            ...
            ensemble_selection.fit(
                predictions=pred_probas, labels=y_internal, identifiers=None
            )  # TODO: Only fit non-nan
            oracle_weights = ensemble_selection.weights_
            oracle_pred_time_start = time.time()
            oracle_pred_proba_norm = [
                pred * weight for pred, weight in zip(pred_probas, oracle_weights)
            ]
        ...
        for model_name, y_pred_proba_internal in model_pred_proba_dict.items():
            if self.eval_metric.needs_pred:
                if self.problem_type == BINARY:
                    # Use 1 and 0, otherwise f1 can crash due to unknown pos_label.
                    y_pred = get_pred_from_proba(
                        y_pred_proba_internal, problem_type=self.problem_type
                    )
                    y_tmp = y_internal
                else:
                    y_pred = self.label_cleaner.inverse_transform_proba(
                        y_pred_proba_internal, as_pred=True
                    )
                    y_tmp = y
            else:
                y_pred = self.label_cleaner.inverse_transform_proba(
                    y_pred_proba_internal, as_pred=False
                )
                y_tmp = y_internal
        ...
        df = pd.DataFrame(
            data={
                ...
                "pred_time_test": [
                    pred_time_test[model] for model in model_names_final
                ],
                "pred_time_test_marginal": [
                    pred_time_test_marginal[model] for model in model_names_final
                ],
            }
        )
    ...
    def _validate_class_labels(self, y: Series):
        ...
        if null_count:
            raise ValueError(
                f"Labels cannot contain missing (nan) values. Found {null_count} missing label values."
            )
    ...
    def evaluate(
        ...
        )  # TODO: Enable DataFrame for y_pred_proba
        ...
        if not high_always_good:
            performance = self.eval_metric.convert_score_to_sklearn_val(
                performance
    ...
    def extract_label(self, X):
        if self.label not in list(X.columns):
            raise ValueError(
                f"Provided DataFrame does not contain label column: {self.label}"
            )
        y = X[self.label].copy()
    ...
    def leaderboard(
        self, X=None, y=None, extra_info=False, only_pareto_frontier=False, silent=False
    ):
        if X is not None:
        ...
        if only_pareto_frontier:
            if (
                "score_test" in leaderboard.columns
                and "pred_time_test" in leaderboard.columns
            ):
            ...
            leaderboard = get_leaderboard_pareto_frontier(
                ...
                score_col=score_col,
                inference_time_col=inference_time_col,
            )
        if not silent:
    ...
    def get_feature_importance(
        ...
        **kwargs,
        ...
        if feature_stage not in valid_feature_stages:
            raise ValueError(
                f"feature_stage must be one of: {valid_feature_stages}, but was {feature_stage}."
            )
        trainer = self.load_trainer()
        if X is not None:
            if y is None:
    ...
    def persist_trainer(
        self, low_memory=False, models="all", with_ancestors=False, max_memory=None
    ) -> list:
        self.trainer = self.load_trainer()
        if not low_memory:
            return self.trainer.persist_models(
                models, with_ancestors=with_ancestors, max_memory=max_memory
            )
