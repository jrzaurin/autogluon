...
def get_preset_custom(name, problem_type, num_classes):
    if not isinstance(name, str):
        raise ValueError(
            f"Expected string value for custom model, but was given {name}"
        )
    # Custom model
    if name == "GBM":
        model = get_param_baseline_custom(problem_type, num_classes=num_classes)
        model[AG_ARGS] = dict(
            model_type="GBM", name_suffix="Large", hyperparameter_tune_kwargs=None
        )
        return [model]
...
def _get_preset_electra(backbone, params_name, name_suffix):
    ...
    model["models"]["BertForTextPredictionBasic"]["search_space"][
        "model.backbone.name"
    ] = backbone
    model["models"]["BertForTextPredictionBasic"]["search_space"][
