...
class LabelCleaner:
    ...
    def construct(
        problem_type: str,
        y: Union[Series, np.ndarray, list],
        y_uncleaned: Union[Series, np.ndarray, list] = None,
        positive_class=None,
    ):
...
class LabelCleanerMulticlass(LabelCleaner):
    def __init__(self, y: Series, y_uncleaned: Series):
        ...
        self.cat_mappings_dependent_var_uncleaned: dict = (
            self._generate_categorical_mapping(y_uncleaned)
        )
        self.inv_map_uncleaned: dict = {
            v: k for k, v in self.cat_mappings_dependent_var_uncleaned.items()
        }
        ...
        self.ordered_class_labels_transformed = list(
            range(len(self.valid_ordered_class_labels))
        )
        self.invalid_class_count = len(self.ordered_class_labels) - len(
            self.valid_ordered_class_labels
        )
        self.labels_to_zero_fill = [
            1 if label not in self.valid_ordered_class_labels else 0
            for label in self.ordered_class_labels
        ]
        self.label_index_to_keep = [
            i for i, label in enumerate(self.labels_to_zero_fill) if label == 0
        ]
        self.label_index_to_remove = [
            i for i, label in enumerate(self.labels_to_zero_fill) if label == 1
        ]
    ...
    def transform_proba(
        self, y: Union[DataFrame, np.ndarray], as_pandas=False
        ...
        if self.invalid_class_count > 0:
            ...
            if isinstance(y, DataFrame):
                cols_to_drop = [
                    self.cat_mappings_dependent_var_uncleaned[col]
                    for col in self.label_index_to_remove
                ]
        ...
        if as_pandas:
            if isinstance(y, DataFrame):
                ...
                return DataFrame(
                    data=y,
                    columns=self.ordered_class_labels_transformed,
                    dtype=np.float32,
                )
    ...
    def inverse_transform_proba(self, y, as_pandas=False, as_pred=False):
        ...
        if as_pred:
            y_transformed = get_pred_from_proba(
                y_pred_proba=y_transformed, problem_type=MULTICLASS
            )
...
class LabelCleanerBinary(LabelCleaner):
    def __init__(self, y: Series, positive_class=None):
        ...
        if len(self.unique_values) != 2:
            raise AssertionError(
                "y does not contain exactly 2 unique values:", self.unique_values
            )
        try:
        ...
        if positive_class is not None:
            ...
            false_val = [val for val in self.unique_values if str(val) == str(False)][
                0
            ]  # may be str or bool
            true_val = [val for val in self.unique_values if str(val) == str(True)][
                0
            ]  # may be str or bool
            ...
            logger.log(
                15,
                "Note: For your binary classification, AutoGluon arbitrarily selects which label-value represents positive vs negative class",
            )
        ...
        logger.log(
            20,
            "Selected class <--> label mapping:  class 1 = %s, class 0 = %s"
            % (poslabel, neglabel),
        )
    ...
    def inverse_transform_proba(self, y, as_pandas=False, as_pred=False):
        ...
        if isinstance(y, Series):
            ...
            y = y.to_numpy()
        if as_pred:
            y = get_pred_from_proba(
                y_pred_proba=y, problem_type=self.problem_type_transform
            )
            y = self._convert_to_valid_series(y)
            y = y.map(self.cat_mappings_dependent_var)
            y = y.to_numpy()
