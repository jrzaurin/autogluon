...
def test_fifo_scheduler():
    @ag.args(
        lr=ag.space.Real(1e-3, 1e-2, log=True), wd=ag.space.Real(1e-3, 1e-2), epochs=10
    )
    ...
    assert pickle.dumps(scheduler.config_history[best_task_id]) == pickle.dumps(
        best_config
    )
...
def test_timeout_scheduler():
    ...
    def foo(args, reporter):
        ...
        for i in range(10):
            # Sleep for 1 second
            time.sleep(1)
            reporter(reward=time.time() - start_tick, time_attr=i)
