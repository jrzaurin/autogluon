...
from ..utils.default_arguments import (
...
class FIFOScheduler(TaskScheduler):
    ...
    def __init__(self, train_fn, **kwargs):
        ...
        if isinstance(searcher, str):
            ...
            self.searcher: BaseSearcher = searcher_factory(searcher, **_search_options)
        else:
        ...
        self.metadata = {
            ...
            "stop_criterion": {"time_limits": time_out, "max_reward": self.max_reward},
        ...
        self.training_history_callback_delta_secs = kwargs[
            "training_history_callback_delta_secs"
        ]
    ...
    def run(self, **kwargs):
        ...
        if num_trials is not None:
            logger.log(
                15, f"Num of Pending Tasks is {num_trials - self.num_finished_tasks}"
            )
    ...
    def _create_new_task(self, config, resources=None):
        if resources is None:
            resources = DistributedResource(**self.resource)
        return Task(
            self.train_fn, {"args": self.args, "config": config}, resources=resources
        )
    ...
    def add_job(self, task, **kwargs):
        ...
        # reporter thread
        rp = threading.Thread(
            target=self._run_reporter, args=(task, job, reporter), daemon=False
        )
        rp.start()
        ...
        # callback
        if self._checkpoint is not None or self.training_history_callback is not None:
    ...
    def _trigger_training_history_callback(self):
        ...
        current_block = int(
            np.floor(self._elapsed_time() / self.training_history_callback_delta_secs)
        )
        current_len = len(self.training_history)
        ret_val = (
            current_block > self._training_history_callback_last_block
        ) and current_len > self._training_history_callback_last_len
        if ret_val:
    ...
    def _elapsed_time(self):
        ...
        """
        assert self._start_time is not None, "Experiment has not been started yet"
        ...
        with self._fifo_lock:
            for task_id, config in self.config_history.items():
                if pickle.dumps(best_config) == pickle.dumps(config):
                    return task_id
            raise RuntimeError(
                "The best config {} is not found in config history = {}. "
        ...
        if self.visualizer == "mxboard" or self.visualizer == "tensorboard":
            if "loss" in reported_result:
                self.mxboard.add_scalar(
                    tag="loss",
                    value=(f"task {task_id} valid_loss", reported_result["loss"]),
    ...
    def get_training_curves(self, filename=None, plot=False, use_legend=True):
        ...
        eval_metric = self.__get_training_history_metric(
            "eval_metric", default="validation_performance"
        )
        sign_mult = (
            int(self.__get_training_history_metric("greater_is_better", default=True))
            * 2
            - 1
        )
