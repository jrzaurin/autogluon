...
def tt_trial(args, reporter):
    ...
    try:
        ...
        reporter(
            epoch=model.params["epochs"] + 1, validation_performance=model.val_score
        )
    pass
...
def augmentation(data, target, **params):
    ...
    locs_to_mask = (
        torch.empty_like(cat_data, dtype=float).uniform_() < params["aug_mask_prob"]
    )
        ...
        if self.encoders is not None:
            ...
            for c in self.columns:
                ...
                if c["type"] == "SCALAR" and col.nunique() < 32:
                    logger.log(
                        15,
                        f"Column {c['name']} shouldn't be encoded as SCALAR. Switching to CATEGORICAL.",
                    )
                    enc = tab_transformer_encoder.__dict__[
