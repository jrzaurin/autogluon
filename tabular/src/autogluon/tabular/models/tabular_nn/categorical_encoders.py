    ...
    def _fit(self, X, handle_unknown="error"):
        ...
        if self.max_levels is not None:
            if not isinstance(self.max_levels, Integral) or self.max_levels <= 0:
                raise ValueError(
                    "max_levels must be None or a strictly "
                    "positive int, got {}.".format(self.max_levels)
                )
...
class OneHotMergeRaresHandleUnknownEncoder(_BaseEncoder):
    ...
    def fit(self, X, y=None):
        ...
        X = np.array(
            X
        ).tolist()  # converts all elements in X to the same type (i.e. cannot mix floats, ints, and str)
    ...
    def transform(self, X):
        ...
        for feature_idx in range(n_features):
            ...
            if self.drop is not None:
                ...
                if (
                    isinstance(self.drop, str)
                    and self.drop == "infrequent"
                    and n_infrequent == 0
                ):
...
class OrdinalMergeRaresHandleUnknownEncoder(_BaseEncoder):
    ...
    def fit(self, X, y=None):
        ...
        self.categories_unknown_level_ = [
            min(len(categories), self.max_levels) for categories in self.categories_
        ]
    ...
    def transform(self, X):
        ...
        X = (
            X_og_array.tolist()
        )  # converts all elements in X to the same type (i.e. cannot mix floats, ints, and str)
        ...
        for i in range(X_int.shape[1]):
            ...
            unknown_elements = np.array(
                [cat not in cat_set for cat in X_col_data.tolist()]
            )
