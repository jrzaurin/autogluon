...
@ag.args(x1=ag.space.Real(-5.0, 10.0), x2=ag.space.Real(0.0, 15.0))
...
@ag.args(x1=ag.space.Real(-5.0, 10.0), x2=ag.space.Real(0.0, 15.0), epochs=9)
...
def to_input(config_dict: dict, hp_ranges: HyperparameterRanges_CS) -> np.ndarray:
...
def assert_same_config_history(results1, results2, num_trials, hp_ranges, decimal=5):
    ...
    config_history1 = remove_duplicate_configs(config_history1, results1["task_id"])
    config_history2 = remove_duplicate_configs(config_history2, results2["task_id"])
...
def test_resume_fifo_random():
    ...
    logger.info(
        "Running [{} - two phases]: Resume from checkpoint, num_trials={}".format(
    ...
    logger.info(
        "Running [{} - one phase]: num_trials={}".format(exper_type, num_trials2)
    )
    scheduler3.run()
