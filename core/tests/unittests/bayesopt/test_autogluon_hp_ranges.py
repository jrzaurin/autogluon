...
from autogluon.core.searcher.bayesopt.datatypes.scaling import LinearScaling, LogScaling
...
def test_to_ndarray():
    ...
    for iter in range(20):
            ...
            if np.random.random() < _prob_categ:
                ...
                if rand_coin < 0.5:
                    ...
                    hp = CSH.UniformFloatHyperparameter(
                        name=name, lower=0.5, upper=5.0, log=log_scaling
                    )
        ...
        for cmp_it in range(5):
            ...
            config = (_config[name] for name in config_space.get_hyperparameter_names())
...
def test_to_ndarray_name_last_pos():
    ...
    config_space.add_hyperparameters(
        [
            CSH.UniformFloatHyperparameter("a", lower=0.0, upper=1.0),
    ...
    hp_a = HyperparameterRangeContinuous(
        "a", lower_bound=0.0, upper_bound=1.0, scaling=LinearScaling()
    )
