...
_ARGUMENT_KEYS = {"objectives", "scalarization_options"}
...
class MOHyperbandScheduler(HyperbandScheduler):
    ...
    def __init__(self, train_fn, **kwargs):
        ...
        assert "_SCALARIZATION" not in kwargs["objectives"], (
            "_SCALARIZATION" "is a protected value. Please refrain from using it."
        )
    ...
    def add_job(self, task, **kwargs):
        ...
        weights = [
            uniform_from_unit_simplex(len(self._objectives)) for _ in range(n_weights)
        ]
        weights = [w * self._sign_vector for w in weights]
        reporter = MODistStatusReporter(
            self._objectives,
            weights,
